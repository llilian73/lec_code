    ...
    for base_path in base_paths:
        ...
        for file in files:
            ...
            try:
                ...
                difference_data['total'][region] = ref_data['total_demand_sum_p(kWh/person)'] - case_data[
                    'total_demand_sum_p(kWh/person)']
                difference_data['heating'][region] = ref_data['heating_demand_sum_p(kWh/person)'] - case_data[
                    'heating_demand_sum_p(kWh/person)']
                difference_data['cooling'][region] = ref_data['cooling_demand_sum_p(kWh/person)'] - case_data[
                    'cooling_demand_sum_p(kWh/person)']
...
def d_reduction_difference_map(difference_data, shapefile_path, save_path, year, case_num):
    ...
    for data_type, differences in difference_data.items():
        ...
        world.plot(column='difference',
                   ...
                   legend=True,
                   legend_kwds={'label': 'Demand Difference (kWh/person)',
                                'orientation': 'horizontal',
                                'shrink': 0.6, 'pad': 0.02},
                   cmap=colormaps[data_type],
                   vmin=vmin,
                   vmax=vmax)
        plt.title(f'{data_type.capitalize()} Demand Difference (kWh/person) - Case {case_num} - {year}', fontsize=16)
        ax.axis('off')
        ...
        for file in files:
            ...
            try:
                ...
                if case_key not in df.index:
                    continue
                case_data = df.loc[case_key]
                reduction_data['total'][region] = case_data['total_demand_p_reduction(%)']
                reduction_data['heating'][region] = case_data['heating_demand_p_reduction(%)']
                reduction_data['cooling'][region] = case_data['cooling_demand_p_reduction(%)']
            except Exception as e:
                print(f"处理{file}时出错: {e}")
                continue
