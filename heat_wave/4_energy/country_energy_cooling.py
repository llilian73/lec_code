...
# --- 配置路径 ---
east_dir = r"C:\Users\thuarchdog\Desktop\result\EAST_output_all_heat_wave"
west_dir = r"C:\Users\thuarchdog\Desktop\result\WEST_output_all_heat_wave"
...
def load_and_merge_data(east_dir, west_dir, case):
    ...
    if os.path.exists(east_csv_path):
        ...
        # 移除包含NaN的行
        east_data = east_data.dropna(subset=['cooling_demand'])
    ...
    if os.path.exists(west_csv_path):
        ...
        # 移除包含NaN的行
        west_data = west_data.dropna(subset=['cooling_demand'])
...
def calculate_national_energy(input_data, shp_path, output_csv_path):
    ...
    try:
        ...
        # 按国家聚合制冷能耗
        national_energy = joined_gdf.groupby('ISO_A2')['cooling_demand'].sum().reset_index()
        national_energy.rename(columns={'ISO_A2': 'country', 'cooling_demand': 'cooling_energy'}, inplace=True)

        # 合并CN和CN-TW为CN
        cn_tw_energy = 0
        if 'CN-TW' in national_energy['country'].values:
            cn_tw_energy = national_energy[national_energy['country'] == 'CN-TW']['cooling_energy'].iloc[0]
        ...
        if 'CN' in national_energy['country'].values:
            ...
            national_energy.loc[cn_idx, 'cooling_energy'] += cn_tw_energy
        else:
            # 如果只有CN-TW没有CN，则创建CN记录
            national_energy = pd.concat(
                [national_energy, pd.DataFrame({'country': ['CN'], 'cooling_energy': [cn_tw_energy]})],
                ignore_index=True)

        print(f"已聚合 {len(national_energy)} 个国家的能耗数据。")
        print(
            f"CN和CN-TW合并后的总能耗: {national_energy[national_energy['country'] == 'CN']['cooling_energy'].iloc[0] if 'CN' in national_energy['country'].values else 0}")
...
def plot_global_heatmap(case_results, shp_path, ref_max_energy, output_pdf_dir):
    ...
    for case_name, df_energy in case_results.items():
        ...
        cn_energy = df_energy[df_energy['country'] == 'CN']['cooling_energy'].iloc[0] if 'CN' in df_energy[
            'country'].values else 0

        # 为CN和CN-TW设置相同的能耗值
        merged_gdf.loc[merged_gdf['ISO_A2'] == 'CN', 'cooling_energy'] = cn_energy
        merged_gdf.loc[merged_gdf['ISO_A2'] == 'CN-TW', 'cooling_energy'] = cn_energy
        ...
        merged_gdf.plot(
            ...
            edgecolor='0.3',
            legend=True,
            legend_kwds={'label': "Cooling Energy Consumption (GW)", 'orientation': "horizontal", 'shrink': 0.8,
                         'pad': 0.02},
            vmin=0,
            vmax=120000  # 使用ref工况的最大值作为颜色条上限
        )

        ax.set_title(f'Global Cooling Energy Consumption ({case_name})', fontsize=16)
        ax.set_axis_off()
        ...
        # 保存为PDF格式
        output_pdf_path = os.path.join(case_output_dir, f'{case_name}_global_cooling_energy_heatmap.pdf')
...
def main():
    ...
    # 创建输出目录
    output_base_dir = r"C:\Users\thuarchdog\Desktop\result\global_energy_output_cooling"
    ...
    for case in cases:
        ...
        if merged_data is None:
            ...
            global_summary[case] = {'cooling_demand_sum(GWh)': None, 'cooling_demand_diff(GWh)': None,
        ...
        # 计算全球总能耗
        current_total_sum = national_energy['cooling_energy'].sum()
        global_summary[case] = {'cooling_demand_sum(GWh)': current_total_sum}
        print(f"工况 {case} 的全球总能耗: {current_total_sum:.2f} GWh")

        # 如果是ref工况，记录最大能耗值
        if case == 'ref':
            ref_max_energy = float(national_energy['cooling_energy'].max())
    ...
    if ref_total_sum > 0:
        for case in cases:
            if case == 'ref':
                global_summary['ref']['cooling_demand_diff(GWh)'] = ''  # ref工况不计算差异和节能率
                global_summary['ref']['cooling_demand_reduction(%)'] = ''
            else:
                current_sum = global_summary[case]['cooling_demand_sum(GWh)']
