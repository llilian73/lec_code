...
输出数据：
- 路径：/home/linbor/WORK/lishiying/heat_wave/{模型名}/{SSP路径}/energy/{年份}/point/
...
def process_single_year(model_name, ssp_path, year):
    ...
    # 输出目录（修改为：energy/{年份}/point/{case_name}）
    ...
    # 将结果写入CSV文件（按case分组）
    logger.info(f"  [保存] 开始将结果写入CSV文件，共 {len(results)} 个点的结果")
    ...
    # 为每个case创建DataFrame并写入CSV
    for case_name, data_list in case_data.items():
        if len(data_list) == 0:
            logger.warning(f"  [警告] Case {case_name} 没有数据，跳过写入")
            continue
        
        df = pd.DataFrame(data_list)
        csv_file = os.path.join(output_dir, f"{case_name}.csv")
        
        try:
            df.to_csv(csv_file, index=False)
        except Exception as e:
            logger.error(f"  [错误] 保存 Case {case_name} 结果失败: {str(e)}")
            import traceback
            traceback.print_exc()
    
    save_elapsed = time.perf_counter() - save_start_time
    logger.info(f"  [完成] {year} 年所有CSV文件写入完成，耗时 {save_elapsed:.2f} 秒，共写入 {len(case_data)} 个case文件")
    
    # 统计处理结果
    total_points_processed = len(results)
    total_cases_written = len(case_data)
