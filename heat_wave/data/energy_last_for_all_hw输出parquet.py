...
- 每个case文件夹包含两个文件：
  ...
  2. ref_hourly.parquet 或 case{number}_hourly.parquet：逐小时能耗（列：lat, lon, number, date, time, total_demand, heating_demand, cooling_demand）
"""
...
# Parquet压缩格式配置
# 可选值: 'snappy' (默认, 速度快), 'gzip' (压缩率高), 'brotli' (压缩率最高), 'lz4' (速度快), 'zstd' (平衡)
...
def process_point(point_data, shared_data, population_df):
    """处理单个点的所有热浪事件，计算总能耗"""
    lat = point_data['lat']
    lon = point_data['lon']
    point_key = f"({lat}, {lon})"
    ...
    try:
        ...
        pop_duration = time.perf_counter() - pop_start
        
        if len(pop_data) == 0:
            logger.warning(f"未找到人口数据: {point_key} (查询耗时: {pop_duration:.4f}s)")
            return None
        ...
        # 存储每个热浪事件的逐小时数据
        hourly_results = {}  # {case_name: [DataFrame, ...]} 每个case对应多个热浪事件的逐小时数据
        
        # 3. 循环处理每个热浪事件
        for idx, heat_wave in enumerate(heat_waves, 1):
            wave_date = heat_wave['date']
            duration = int(heat_wave['Duration'])
            
            try:
                # 3.1 解析日期
                month, day = map(int, wave_date.split('/'))
                ...
                # 3.3.1 保存逐小时数据（用于后续输出）
                # results[case_name] 是 DataFrame，索引是逐小时时间，列包括 total_demand, heating_demand, cooling_demand
                for case_name, case_result in results.items():
                    # case_result 是 DataFrame，索引是逐小时时间戳
                    hourly_df = case_result[['total_demand', 'heating_demand', 'cooling_demand']].copy()
                    hourly_df['lat'] = lat
                    hourly_df['lon'] = lon
                    hourly_df['number'] = idx  # 热浪事件编号
                    hourly_df['date'] = hourly_df.index.date  # 日期（不含时间）
                    hourly_df['time'] = hourly_df.index.time  # 时间（不含日期）
                    # 重置索引，将时间信息转换为列
                    hourly_df = hourly_df.reset_index(drop=True)
                    hourly_df = hourly_df[['lat', 'lon', 'number', 'date', 'time', 'total_demand', 'heating_demand', 'cooling_demand']]
                    
                    if case_name not in hourly_results:
                        hourly_results[case_name] = []
                    hourly_results[case_name].append(hourly_df)
                ...
                logger.error(f"处理热浪事件失败: {point_key}, 日期: {wave_date}, 错误: {str(e)}", exc_info=True)
                continue
        ...
        result_with_coords = {
            'lat': lat,
            'lon': lon,
            'results': total_results,  # 总能耗结果
            'hourly_results': hourly_results  # 逐小时能耗数据 {case_name: [DataFrame, ...]}
        }
        
        # 函数总耗时
        func_duration = time.perf_counter() - func_start_time
        # logger.info(f"点处理完成: {point_key}, 总耗时: {func_duration:.4f}s, "
                   #
        return result_with_coords
    
    except Exception as e:
        func_duration = time.perf_counter() - func_start_time
        logger.error(f"处理点 {point_key} 失败, 总耗时: {func_duration:.4f}s, 错误: {str(e)}", exc_info=True)
        import traceback
        traceback.print_exc()
        return None
...
def init_worker(shared_data_dict, population_df_dict):
    ...
    try:
        ...
        logger.error(f"[进程 {worker_id}] 工作进程初始化失败: {str(e)}")
        import traceback
        traceback.print_exc()
        raise
...
def process_batch(batch_points):
    ...
    # logger.info(f"[进程 {worker_id}] 开始处理批次，包含 {batch_size} 个点")
    
    try:
        batch_results = []
        point_count = 0
        
        for idx, point_data in enumerate(batch_points):
            point_start_time = time.perf_counter()
            try:
                # logger.info(f"[进程 {worker_id}] 处理点 {idx+1}/{batch_size}: lat={point_data['lat']}, lon={point_data['lon']}")
                result = process_point(point_data, _global_shared_data, _global_population_df)
                if result:
                    batch_results.append(result)
                    point_count += 1
                point_elapsed = time.perf_counter() - point_start_time
                # logger.info(f"[进程 {worker_id}] 点 {idx+1} 处理完成，耗时 {point_elapsed:.2f} 秒")
            except Exception as e:
                point_elapsed = time.perf_counter() - point_start_time
                logger.error(f"[进程 {worker_id}] 处理点 {idx+1} 出错 (耗时 {point_elapsed:.2f} 秒): {str(e)}")
                import traceback
                traceback.print_exc()
                continue
        
        batch_elapsed = time.perf_counter() - batch_start_time
        # logger.info(f"[进程 {worker_id}] 批次处理完成: 成功 {point_count}/{batch_size} 个点，总耗时 {batch_elapsed:.2f} 秒")
        return batch_results
    except Exception as e:
        batch_elapsed = time.perf_counter() - batch_start_time
        logger.error(f"[进程 {worker_id}] 批次处理出错 (耗时 {batch_elapsed:.2f} 秒): {str(e)}")
        import traceback
        traceback.print_exc()
        return []
...
def process_single_year(model_name, ssp_path, year):
    ...
    # 根据SSP路径选择对应的人口文件
    # SSP126 -> SSP1_population.csv, SSP245 -> SSP2_population.csv
    ssp_to_population = {
        "SSP126": "SSP1_population.csv",
        "SSP245": "SSP2_population.csv"
    }
    
    if ssp_path not in ssp_to_population:
        raise ValueError(f"不支持的SSP路径: {ssp_path}，支持: {list(ssp_to_population.keys())}")
    
    population_file = os.path.join(POPULATION_BASE_PATH, ssp_to_population[ssp_path])
    
    if not os.path.exists(population_file):
        raise FileNotFoundError(f"人口文件不存在: {population_file}")
    
    #     logger.info(f"  [加载] 人口文件: {os.path.basename(population_file)}")
    ...
    for var_name, (shm, array) in shared_data.data.items():
        shared_data_dict['data'][var_name] = {
            'shm_name': shm.name,
            'shape': array.shape,
            'dtype': array.dtype
        }
    ...
    logger.info(f"  [并行] 使用 {NUM_PROCESSES} 个进程处理，总点数={total_points}, 批次大小={batch_size}, 批次数={len(batches)}")
    ...
    try:
        with multiprocessing.Pool(
            ...
            # logger.info(f"进程池创建成功，chunksize={chunksize}")
            
            # 显示进度条，包含SSP路径和年份信息
            with tqdm(total=len(batches), desc=f"  [{ssp_path}-{year}]", 
                     bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]') as pbar:
                completed_batches = 0
                for batch_results in pool.imap_unordered(process_batch, batches, chunksize=chunksize):
                    completed_batches += 1
                    elapsed = time.perf_counter() - parallel_start_time
                    if batch_results:
                        results.extend(batch_results)
                    pbar.update(1)
            
            parallel_elapsed = time.perf_counter() - parallel_start_time
            logger.info(f"  [完成] {year} 年并行处理完成，耗时 {parallel_elapsed:.1f} 秒，成功处理 {len(results)}/{total_points} 个点")
    except Exception as e:
        elapsed = time.perf_counter() - parallel_start_time
        logger.error(f"并行处理出错 (耗时 {elapsed:.1f} 秒): {str(e)}")
        import traceback
        traceback.print_exc()
        raise
    
    # 将结果写入文件（按case分组）：总能耗为CSV，逐小时为Parquet
    logger.info(f"  [保存] 开始将结果写入文件，共 {len(results)} 个点的结果")
    save_start_time = time.perf_counter()
    
    # 按case组织总能耗数据
    case_data = {}  # {case_name: [{'lat': ..., 'lon': ..., 'total_demand': ..., ...}, ...]}
    # 按case组织逐小时数据
    case_hourly_data = {}  # {case_name: [DataFrame, ...]} 每个case的所有点的逐小时数据列表
    
    for point_result in results:
        if point_result is None:
            continue
        lat = point_result['lat']
        lon = point_result['lon']
        results_dict = point_result['results']
        hourly_results = point_result.get('hourly_results', {})  # 获取逐小时数据
        
        # 处理总能耗数据
        for case_name, case_result in results_dict.items():
        ...
        # 处理逐小时数据
        for case_name, hourly_dfs in hourly_results.items():
            if case_name not in case_hourly_data:
                case_hourly_data[case_name] = []
            case_hourly_data[case_name].extend(hourly_dfs)
    
    # 为每个case创建文件夹并写入两个CSV文件
    for case_name in set(list(case_data.keys()) + list(case_hourly_data.keys())):
        # 创建case文件夹
        case_dir = os.path.join(output_dir, case_name)
        os.makedirs(case_dir, exist_ok=True)
        
        # 1. 保存总能耗CSV
        if case_name in case_data and len(case_data[case_name]) > 0:
            df_total = pd.DataFrame(case_data[case_name])
            csv_file_total = os.path.join(case_dir, f"{case_name}.csv")
            try:
                df_total.to_csv(csv_file_total, index=False)
            except Exception as e:
                logger.error(f"  [错误] 保存 Case {case_name} 总能耗结果失败: {str(e)}")
                import traceback
                traceback.print_exc()
        else:
            logger.warning(f"  [警告] Case {case_name} 没有总能耗数据，跳过写入")
        
        # 2. 保存逐小时Parquet
        if case_name in case_hourly_data and len(case_hourly_data[case_name]) > 0:
            # 合并所有DataFrame
            df_hourly = pd.concat(case_hourly_data[case_name], ignore_index=True)
            
            # 确定逐小时文件名（与总能耗文件命名逻辑一致，统一用 {case_name}_hourly.parquet）
            parquet_file_hourly = os.path.join(case_dir, f"{case_name}_hourly.parquet")
            try:
                df_hourly.to_parquet(parquet_file_hourly, index=False, engine='pyarrow', compression=PARQUET_COMPRESSION)
            except Exception as e:
                logger.error(f"  [错误] 保存 Case {case_name} 逐小时结果失败: {str(e)}")
                import traceback
                traceback.print_exc()
        else:
            logger.warning(f"  [警告] Case {case_name} 没有逐小时数据，跳过写入")
    
    save_elapsed = time.perf_counter() - save_start_time
    total_cases = len(set(list(case_data.keys()) + list(case_hourly_data.keys())))
    logger.info(f"  [完成] {year} 年所有文件写入完成，耗时 {save_elapsed:.2f} 秒，共写入 {total_cases} 个case文件夹")
    
    # 统计处理结果
    total_points_processed = len(results)
    total_cases_written = total_cases
    
    # 清理资源
    shared_data.cleanup()
    del grouped_points, results, case_data
    gc.collect()
    
    logger.info(f"  [完成] ✓ {year} 年处理完成，成功处理 {total_points_processed} 个点，写入 {total_cases_written} 个case文件\n")
