...
# 输入路径：使用linear_interpolation_lin.py的输出（GCM_input_processed）
...
def main():
    """主函数"""
    try:
        logger.info("=== 开始热浪识别处理 ===")
        
        # 检查路径
        if not os.path.exists(MODEL_BASE_PATH):
            raise FileNotFoundError(f"模型数据路径不存在: {MODEL_BASE_PATH}")
        ...
        # 查找历史数据文件（需要包含_interpolated_1deg后缀）
        ...
        if not historical_files:
            raise FileNotFoundError(f"未找到历史数据文件（需要包含_interpolated_1deg）: {historical_dir}")
        
        historical_file = os.path.join(historical_dir, historical_files[0])
        logger.info(f"使用历史数据文件: {historical_file}")
        
        # 识别陆地点
        land_points = identify_land_points(historical_file, world_data)
        if not land_points:
            logger.warning("未找到任何陆地点，程序退出")
            return
        ...
        for ssp_path in SSP_PATHS:
            ...
            # 文件格式：tasmax_day_{MODEL_NAME}_{ssp}_r1i1p1f1_gn_20300101-20341231_interpolated_1deg.nc
            # 支持两种格式：每年一个文件，或多年合并在一个文件
            future_files = {}
            all_nc_files = [
            ...
            # 并行处理（使用init_worker初始化每个进程的数据，传递共享内存信息）
            with multiprocessing.Pool(
                processes=num_processes,
                initializer=init_worker,
                initargs=(historical_file, future_files, shm.name, data_shape, data_dtype,
                         time_index, lat_index, lon_index)
            ) as pool:
                # 优化chunksize，减少进程间通信开销
                chunksize = max(1, len(batches) // (num_processes * 2))
                for batch_results in tqdm(
                    pool.imap_unordered(process_batch, batches, chunksize=chunksize),
                    total=len(batches),
                    desc=f"处理批次 ({ssp_path})"
                ):
                    # 按年份分类结果
