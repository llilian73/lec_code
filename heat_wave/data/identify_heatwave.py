...
#     "EC-Earth3",
# 已处理的模型（跳过处理）
PROCESSED_MODELS = ["BCC-CSM2-MR", "ACCESS-ESM1-5","MPI-ESM1-2-HR", "MRI-ESM2-0"]
...
def load_year_data(year):
    ...
    if file_path not in _global_future_file_cache:
        # 使用chunks延迟加载，避免一次性加载整个文件的时间索引
        _global_future_file_cache[file_path] = xr.open_dataset(file_path, chunks={'time': 400})
...
def process_single_point(point_info):
    ...
    try:
        ...
        # 处理每个目标年份（按年加载）
        all_results = []
        for year in _global_future_files.keys():
            # 按需加载该年份的数据
...
def verify_historical_data(historical_files, model_name):
    """
    验证历史数据是否包含完整的1981-2010年数据（所有模型统一处理）
...
def process_single_model(model_name, world_data):
    ...
    # 输出路径与输入文件夹匹配（例如：future_56_60 -> heat_wave_56_60）
    output_folder_name = f"heat_wave_{FUTURE_FOLDER.replace('future_', '')}"
    output_base_path = os.path.join(BASE_PATH, output_folder_name, model_name)
    ...
    for ssp_path in SSP_PATHS:
        ...
        # 查找未来数据文件
        future_dir = os.path.join(model_base_path, FUTURE_FOLDER, ssp_path)
        ...
        # 动态显示目标年份范围
        target_years_str = f"{min(TARGET_YEARS)}-{max(TARGET_YEARS)}"
        logger.info(f"验证 {len(tasmax_files)} 个tasmax文件是否包含完整的{target_years_str}年数据...")
        ...
        for nc_file in tasmax_files:
            ...
            try:
                # 从文件名中提取年份范围（用于显示）
                file_years_str = "未知"
                match = re.search(r'(\d{8})-(\d{8})', nc_file)
                if match:
                    start_date = match.group(1)
                    end_date = match.group(2)
                    start_year = int(start_date[:4])
                    end_year = int(end_date[:4])
                    file_years_str = f"{start_year}-{end_year}"
                ...
                with xr.open_dataset(file_path) as ds:
                    ...
                    if missing_years:
                        ...
                        logger.error(f"❌ 文件 {nc_file} (文件名年份范围: {file_years_str}) 缺少年份: {sorted(missing_years)}")
                        logger.error(f"   实际包含年份: {actual_years}")
                        logger.error(f"   目标年份: {TARGET_YEARS}")
                    else:
                        logger.info(f"✓ 文件 {nc_file} (文件名年份范围: {file_years_str}) 包含完整目标年份: {actual_years}")
        ...
        logger.info(f"✓ 所有文件验证通过，都包含完整的{target_years_str}年数据")
