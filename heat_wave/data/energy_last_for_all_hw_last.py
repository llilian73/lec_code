...
输入数据：
1. 热浪事件CSV文件：/home/linbor/WORK/lishiying/{HEAT_WAVE_FOLDER}/{模型名}/{SSP路径}/{年份}_all_heat_wave.csv
   （HEAT_WAVE_FOLDER根据FUTURE_FOLDER自动生成，例如：future_56_60 -> heat_wave_56_60）
2. 未来气候数据：/home/linbor/WORK/lishiying/GCM_input_processed/{模型名}/{FUTURE_FOLDER}/{SSP路径}/
   （FUTURE_FOLDER可配置，例如：future_51_55, future_56_60）
3. 人口数据：/home/linbor/WORK/lishiying/population/SSP1_population.csv (SSP126) 或 SSP2_population.csv (SSP245)

输出数据：
- 路径：/home/linbor/WORK/lishiying/{HEAT_WAVE_FOLDER}/{模型名}/{SSP路径}/energy/{年份}/point/{case_name}/
- 每个case文件夹包含两个CSV文件：
...
# 配置日志
logging.basicConfig(
    level=logging.INFO,  # 使用 INFO 级别，耗时统计用 INFO 输出
...
# 未来数据文件夹配置（可根据需要修改，例如："future_51_55", "future_56_60"）
FUTURE_FOLDER = "future_56_60"

# 根据未来数据文件夹自动生成热浪数据路径（例如：future_56_60 -> heat_wave_56_60）
HEAT_WAVE_FOLDER = f"heat_wave_{FUTURE_FOLDER.replace('future_', '')}"
HEAT_WAVE_BASE_PATH = os.path.join(BASE_PATH, HEAT_WAVE_FOLDER)
...
TARGET_YEARS = [2056, 2057, 2058, 2059, 2060]  # 可根据需要修改，例如：[2051, 2052, 2053, 2054, 2055] 或 [2056, 2057, 2058, 2059, 2060]
...
def find_climate_file(future_dir, variable, model_name, ssp_path):
    ...
    if len(matching_files) == 0:
        ...
        # 多个匹配文件：优先选择包含目标年份范围的文件
        # 构建目标年份的日期范围字符串（例如：20560101-20601231）
        target_start_date = f"{min(TARGET_YEARS)}0101"
        target_end_date = f"{max(TARGET_YEARS)}1231"
        
        for f in matching_files:
            if target_start_date in f and target_end_date in f:
                return os.path.join(future_dir, f)
        
        # 如果找不到完全匹配的，返回字典序第一个（因已 sort）
        logger.warning(f"找到多个匹配文件，未找到包含{target_start_date}-{target_end_date}的文件，使用第一个: {matching_files[0]}")
        return os.path.join(future_dir, matching_files[0])
...
def process_point(point_data, shared_data, population_df, model_name, ssp_path, year):
    ...
    import os
    global _worker_hourly_dir, _global_population_dict
    # === 新增：防御性检查 ===
    ...
    try:
        # 1. 获取人口（优化：使用字典O(1)查询）
        t0 = time.perf_counter()
        population = _global_population_dict.get((lat, lon))
        ...
        # 初始化逐时数据缓冲区（每个case对应一个Table列表）
        hourly_buffers = {f'case{i}': [] for i in range(1, 21)}
        hourly_buffers['ref'] = []
        ...
        for idx, heat_wave in enumerate(heat_waves, 1):
            heatwave_start = time.perf_counter()
            try:
                # 解析日期 & 加载数据（优化：使用预先解析的日期）
                t0 = time.perf_counter()
                if 'parsed_date' in heat_wave:
                    start_date = heat_wave['parsed_date']
                else:
                    # 兼容旧格式：如果没有预先解析，则手动解析
                ...
                t_load_climate = time.perf_counter() - t0

                # 计算能耗
                t0 = time.perf_counter()
                results = calculate_energy_demand(point_data, weather_df, population)
                t_calc_energy = time.perf_counter() - t0

                # === 累积逐时数据到内存缓冲区（不立即写入）===
                # 优化：减少DataFrame复制，使用字典构建数据
                t_hourly_start = time.perf_counter()
                if save_hourly:
                    for case_name, hourly_df in results.items():
                        try:
                            # 优化：直接从DataFrame提取数据，减少复制
                            # 使用字典构建，避免多次DataFrame操作
                            data_dict = {
                                'lat': [lat] * len(hourly_df),
                                'lon': [lon] * len(hourly_df),
                                'number': [idx] * len(hourly_df),
                                'date': pd.to_datetime([d.date() for d in hourly_df.index]),
                                'time': [t.time() for t in hourly_df.index],
                                'total_demand': hourly_df['total_demand'].values,
                                'heating_demand': hourly_df['heating_demand'].values,
                                'cooling_demand': hourly_df['cooling_demand'].values
                            }
                            # 一次性创建DataFrame
                            df = pd.DataFrame(data_dict)
                            
                            # 转为 Arrow Table 并添加到缓冲区（不写入文件）
                            table = pa.Table.from_pandas(df, schema=HOURLY_SCHEMA, preserve_index=False)
                            hourly_buffers[case_name].append(table)
                        except Exception as e:
                            logger.error(f"[ERROR] 处理 case {case_name} 逐小时数据失败: {e}", exc_info=True)
                            raise
                t_hourly_total = time.perf_counter() - t_hourly_start
                ...
                t_heatwave = time.perf_counter() - heatwave_start
                t_heatwaves_total += t_heatwave
                logger.debug(f"[耗时统计] 点 {point_key} 热浪#{idx}: 总耗时={t_heatwave:.3f}s | "
                           f"解析={t_parse:.3f}s | 加载气候={t_load_climate:.3f}s | 计算能耗={t_calc_energy:.3f}s | "
                           f"逐小时写入={t_hourly_total:.3f}s | 累加={t_accumulate:.3f}s")
        ...
        logger.debug(f"[耗时统计] 点 {point_key} 总耗时: {duration:.3f}s | "
                   f"人口查询={t_pop:.3f}s | 初始化={t_init:.3f}s | "
                   f"热浪处理总计={t_heatwaves_total:.3f}s ({len(heat_waves)}个事件, 平均{t_heatwaves_total/len(heat_waves):.3f}s/事件)")
        
        # 返回结果，包含逐时数据缓冲区（不在这里写入文件）
        return {
            'lat': lat, 
            'lon': lon, 
            'results': total_results,
            'hourly_buffers': hourly_buffers if save_hourly else None  # 返回缓冲区供批次级写入
        }
...
_global_population_dict = None  # 优化：使用字典进行O(1)查询


def init_worker(shared_data_dict, population_df_dict):
    """初始化工作进程（+ 创建私有 hourly 输出目录）"""
    import os, tempfile
    global _global_shared_data, _global_population_df, _global_population_dict, _worker_hourly_dir
    ...
    try:
        ...
        # 2. 加载人口数据（优化：同时创建字典用于快速查询）
        _global_population_df = pd.DataFrame(population_df_dict)
        # 创建字典 {(lat, lon): population} 用于O(1)查询
        _global_population_dict = {
            (row['lat'], row['lon']): row['population']
            for _, row in _global_population_df.iterrows()
        }
...
def process_batch(batch_points, model_name, ssp_path, year):
    ...
    # logger.info(f"[进程 {worker_id}] 开始处理批次: {batch_size} 个点")  # 注释：使用整体进度条，不打印进程日志
    
    # 批次级逐时数据缓冲区（累积整个批次的所有点数据）
    batch_hourly_buffers = {}  # {case_name: [table1, table2, ...]}
    
    # 处理批次内的所有点
    for point_idx, point_data in enumerate(batch_points, 1):
        try:
            ...
            if res:
                ...
                # 累积该点的逐时数据到批次缓冲区
                if res.get('hourly_buffers'):
                    for case_name, tables in res['hourly_buffers'].items():
                        if tables:  # 如果有数据
                            if case_name not in batch_hourly_buffers:
                                batch_hourly_buffers[case_name] = []
                            batch_hourly_buffers[case_name].extend(tables)
            
            # 每处理10个点或最后一个点时，输出进度（注释：使用整体进度条，不打印进程日志）
            # if point_idx % 10 == 0 or point_idx == batch_size:
            #     elapsed = time.perf_counter() - batch_start
            #     logger.info(f"[进程 {worker_id}] 批次进度: {point_idx}/{batch_size} 个点 ({point_idx*100//batch_size}%), "
            #               f"已耗时 {elapsed:.1f}s, 成功 {len(batch_results)} 个")
        except Exception as e:
            logger.error(f"[ERROR] 点处理失败: {e}", exc_info=True)
    
    # === 批次处理完成后，一次性写入所有累积的逐时数据 ===
    t_batch_write_start = time.perf_counter()
    if _worker_hourly_dir and batch_hourly_buffers:
        try:
            for case_name, tables in batch_hourly_buffers.items():
                if not tables:  # 跳过空缓冲区
                    continue
                try:
                    # 合并批次内所有点的数据
                    ...
                    if os.path.exists(parquet_path):
                        ...
                        final_table = pa.concat_tables([existing_table, merged_table])
                    else:
                    ...
                    logger.error(f"[ERROR] 批次写入 case {case_name} 逐小时数据失败: {e}", exc_info=True)
                    raise
            
            # 清理批次缓冲区
            del batch_hourly_buffers
            gc.collect()
        except Exception as e:
            logger.error(f"[ERROR] 批次逐时数据写入失败: {e}", exc_info=True)
            raise
    
    t_batch_write_total = time.perf_counter() - t_batch_write_start
    
    # 清理返回结果中的逐时数据缓冲区（避免序列化大对象）
    for res in batch_results:
        if res and 'hourly_buffers' in res:
            del res['hourly_buffers']
    
    batch_duration = time.perf_counter() - batch_start
    # logger.info(f"[进程 {worker_id}] 批次处理完成: {len(batch_results)}/{len(batch_points)} 个点成功, "
    #            f"总耗时={batch_duration:.3f}s, 逐时写入耗时={t_batch_write_total:.3f}s")  # 注释：使用整体进度条，不打印进程日志
...
def process_single_year(model_name, ssp_path, year):
    ...
    # 如果输出目录已存在，先删除其中的所有文件和子目录
    if os.path.exists(output_dir):
        logger.info(f"  [清理] 删除已存在的输出目录内容: {output_dir}")
        try:
            shutil.rmtree(output_dir)
        except Exception as e:
            logger.warning(f"  [警告] 删除目录失败: {str(e)}，将继续尝试创建目录")
    
    # 创建输出目录
    os.makedirs(output_dir, exist_ok=True)
    
    future_dir = os.path.join(CLIMATE_BASE_PATH, model_name, FUTURE_FOLDER, ssp_path)
    ...
    # === 优化：预先解析日期 ===
    # 将日期字符串转换为datetime对象，避免在process_point中重复解析
    ...
    if 'date' in points_df.columns:
        points_df['parsed_date'] = points_df['date'].apply(lambda x: parse_date(x, year))
    ...
    for (lat, lon), group in points_df.groupby(['lat', 'lon']):
        ...
        for _, row in group.iterrows():
            record = row.to_dict()
            # parsed_date已经在DataFrame中，直接包含在记录中
            if 'parsed_date' in record:
                # 确保parsed_date是datetime对象
                if isinstance(record['parsed_date'], pd.Timestamp):
                    record['parsed_date'] = record['parsed_date'].to_pydatetime()
            heat_waves_records.append(record)
        
        grouped_points.append({
            'lat': lat, 'lon': lon,
            'heat_waves': heat_waves_records
        })
    ...
    batch_size = max(1, min(100, total // (NUM_PROCESSES * 8)))
    ...
    try:
        with multiprocessing.Pool(
            ...
            # 使用整体进度条，显示批次进度和已处理点数（每20个批次更新一次）
            with tqdm(total=len(batches), desc=f"  [{ssp_path}-{year}]", 
                      bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]') as pbar:
                completed_points = 0
                completed_batches = 0
                last_update = 0  # 上次更新的批次数
                for batch_res in pool.imap_unordered(batch_func, batches, chunksize=max(1, len(batches)//(NUM_PROCESSES*2))):
                    if batch_res:
                        results.extend(batch_res)
                        completed_points += len(batch_res)
                    completed_batches += 1
                    # 每20个批次或最后一个批次时更新进度条
                    if completed_batches % 20 == 0 or completed_batches == len(batches):
                        update_count = completed_batches - last_update
                        pbar.update(update_count)
                        last_update = completed_batches
                        # 更新进度条描述，显示已处理点数（轻量级操作，不影响性能）
                        pbar.set_postfix({'点数': completed_points})
                    
        logger.info(f"  [✓] 并行完成，耗时 {time.perf_counter() - parallel_start:.1f}s，{len(results)} 点")
        
        # with 语句会自动关闭进程池，但显式清理可以确保资源释放
        # 注意：在 with 块内，pool 会在退出时自动 close() 和 join()
    except Exception as e:
    ...
    # === 【关键】合并 hourly Parquet（优化：使用多线程并行读取）===
    logger.info(f"  [(hourly)] 合并各进程临时 Parquet...")
    try:
        temp_base = "/tmp"
        all_dirs = os.listdir(temp_base)
        temp_dirs = [os.path.join(temp_base, d) for d in all_dirs
                     if d.startswith(f"energy_hourly_{model_name}_{ssp_path}_{year}_pid")]
    except Exception as e:
        logger.error(f"[ERROR] 查找临时目录失败: {e}", exc_info=True)
        temp_dirs = []
    ...
    if not temp_dirs:
        ...
        def read_parquet_file(temp_dir, case):
            """读取单个parquet文件"""
        ...
        for case in [f'case{i}' for i in range(1, 21)] + ['ref']:
            # 使用多线程并行读取所有进程的parquet文件
            tables = []
            with ThreadPoolExecutor(max_workers=min(len(temp_dirs), 8)) as executor:
                futures = {
                    executor.submit(read_parquet_file, temp_dir, case): temp_dir
    ...
    # 清理大对象
    del results, case_data, grouped_points, population_df
    if 'shared_data_dict' in locals():
        del shared_data_dict
    
    # 强制垃圾回收，确保内存完全释放
    gc.collect()
    gc.collect()  # 两次调用确保完全清理
...
def main():
    ...
    try:
        ...
        logger.info(f"未来数据文件夹: {FUTURE_FOLDER}")
        logger.info(f"热浪数据文件夹: {HEAT_WAVE_FOLDER}")
        logger.info(f"目标年份: {TARGET_YEARS} ({min(TARGET_YEARS)}-{max(TARGET_YEARS)})")
