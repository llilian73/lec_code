...
def load_single_parquet_for_preprocess(args):
    """并行读取单个parquet文件（用于预处理阶段）
    
    Parameters:
    -----------
    args : tuple
        (case_name, parquet_path)
    
    Returns:
    --------
    tuple: (case_name, df_hourly) 或 (case_name, None) 如果失败
    """
    case_name, parquet_path = args
    ...
    try:
        ...
        if 'date' in df_hourly.columns and 'time' in df_hourly.columns:
            try:
                ...
                logger.error(f"处理datetime时出错: {e}")
                return (case_name, None)
        ...
        # 添加case列用于区分工况
        df_hourly['case'] = case_name
        
        # 只保留必要列
        needed_cols = ['point_key', 'datetime', 'cooling_demand', 'case']
...
def preprocess_parquet_to_shared_memory(model_name, ssp_path, year, point_to_country):
    """预处理：读取所有工况的parquet，合并后按国家分组，存储在共享内存中
    ...
    Returns:
    --------
    dict: {country_code: shared_memory_info} 每个国家的共享内存信息
        shared_memory_info格式：
        {
            'point_key': list,
            'datetime_sec': {'type': 'shm', 'name': str, 'shape': tuple, 'dtype': str},
            'cooling_demand': {'type': 'shm', 'name': str, 'shape': tuple, 'dtype': str},
            'case': {'type': 'shm', 'name': str, 'shape': tuple, 'dtype': str},
            '_shm_refs': list  # 共享内存引用列表
        }
    """
    preprocess_start = time.time()
    logger.info(f"开始预处理 {model_name} - {ssp_path} - {year} 年的parquet数据到共享内存...")
    
    # 步骤1：并行读取所有工况的parquet文件
    logger.info("步骤1: 读取所有工况的parquet文件...")
    ...
    for case_name in CASES:
        ...
        if os.path.exists(parquet_path):
            parquet_args.append((case_name, parquet_path))
    
    if not parquet_args:
        logger.warning(f"没有找到任何parquet文件")
        return {}
    
    # 并行读取
    read_processes = min(len(parquet_args), 28, os.cpu_count())
    all_case_data = {}  # {case_name: df}
    
    with Pool(processes=read_processes) as read_pool:
        read_results = read_pool.map(load_single_parquet_for_preprocess, parquet_args)
    
    for case_name, df_case in read_results:
        if df_case is not None:
            all_case_data[case_name] = df_case
            logger.info(f"  {case_name}: 读取了 {len(df_case)} 条记录")
    
    if not all_case_data:
        logger.warning("没有成功读取任何工况数据")
        return {}
    
    read_elapsed = time.time() - read_start
    logger.info(f"步骤1完成，耗时: {read_elapsed:.2f} 秒")
    
    # 步骤2：流式合并并按国家分组（避免全量 combined_df，防止 OOM）
    logger.info("步骤2: 流式合并并按国家分组...")
    merge_start = time.time()
    
    # 使用 itertools.chain 流式处理，避免 pd.concat 导致的内存峰值
    all_iterables = (df.iterrows() for df in all_case_data.values())
    combined_iter = chain.from_iterable(all_iterables)
    
    # 流式构建 {country_code: List of records}
    country_records = defaultdict(list)  # {country_code: [(datetime, cooling_demand, case, lat, lon), ...]}
    total_records = 0
    
    for _, row in combined_iter:
        pk = row['point_key']
        country_code = point_to_country.get(pk, (None, None))[0]
        if country_code:
            # 提取 lat 和 lon
            lat, lon = pk
            country_records[country_code].append((
                row['datetime'],
                row['cooling_demand'],
                row['case'],
                lat,
    ...
    logger.info(f"流式处理完成，共 {total_records} 条记录，涉及 {len(country_records)} 个国家")
    merge_elapsed = time.time() - merge_start
    logger.info(f"步骤2完成，耗时: {merge_elapsed:.2f} 秒")
    
    # 步骤3：每个国家独立处理：转换为 numpy 数组并存入共享内存
    logger.info("步骤3: 按国家转换为共享内存...")
    shm_start = time.time()
    country_shared_data = {}  # {country_code: shared_memory_info}
    
    # 建立 case 到整数的映射（全局统一）
    all_cases = set()
    for records in country_records.values():
        for record in records:
            all_cases.add(record[2])  # case 在索引2
    case_names_list = sorted(list(all_cases))  # 排序保证一致性
    case_to_int = {case: idx for idx, case in enumerate(case_names_list)}
    
    total_countries = len(country_records)
    
    for country_idx, (country_code, records) in enumerate(country_records.items(), 1):
        # 转换为共享内存格式
        shared_info = {}
        shm_refs = []
        
        # 提取数据
        datetime_list = [r[0] for r in records]
        cooling_demand_list = [r[1] for r in records]
        case_list = [r[2] for r in records]
        lat_list = [r[3] for r in records]
        lon_list = [r[4] for r in records]
        
        # 1. lat 和 lon: 拆分为两个 float32 数组，存入共享内存
        lat_arr = np.array(lat_list, dtype=np.float32)
        lon_arr = np.array(lon_list, dtype=np.float32)
        
        shm_name_lat = f"year{year}_{country_code[:10]}_lat"
        try:
            shm_lat = shared_memory.SharedMemory(create=True, size=lat_arr.nbytes, name=shm_name_lat)
        except FileExistsError:
            shm_name_lat = f"year{year}_{country_code[:8]}_lat_{uuid.uuid4().hex[:6]}"
            shm_lat = shared_memory.SharedMemory(create=True, size=lat_arr.nbytes, name=shm_name_lat)
        shm_refs.append(shm_lat)
        shared_lat_arr = np.ndarray(lat_arr.shape, dtype=lat_arr.dtype, buffer=shm_lat.buf)
        shared_lat_arr[:] = lat_arr[:]
        ...
        shm_name_lon = f"year{year}_{country_code[:10]}_lon"
        try:
            shm_lon = shared_memory.SharedMemory(create=True, size=lon_arr.nbytes, name=shm_name_lon)
        except FileExistsError:
            shm_name_lon = f"year{year}_{country_code[:8]}_lon_{uuid.uuid4().hex[:6]}"
            shm_lon = shared_memory.SharedMemory(create=True, size=lon_arr.nbytes, name=shm_name_lon)
        shm_refs.append(shm_lon)
        shared_lon_arr = np.ndarray(lon_arr.shape, dtype=lon_arr.dtype, buffer=shm_lon.buf)
        shared_lon_arr[:] = lon_arr[:]
        
        shared_info['lon'] = {
            'type': 'shm',
            'name': shm_lon.name,
            'shape': lon_arr.shape,
            'dtype': str(lon_arr.dtype)
        }
        
        # 2. datetime: 转换为秒级时间戳，存入共享内存
        datetime_series = pd.to_datetime(datetime_list)
        datetime_s = (datetime_series.astype('int64') // 1_000_000_000).astype('int64')
        shm_name_dt = f"year{year}_{country_code[:10]}_dt"
        try:
            shm_dt = shared_memory.SharedMemory(create=True, size=datetime_s.nbytes, name=shm_name_dt)
        except FileExistsError:
            shm_name_dt = f"year{year}_{country_code[:8]}_dt_{uuid.uuid4().hex[:6]}"
            shm_dt = shared_memory.SharedMemory(create=True, size=datetime_s.nbytes, name=shm_name_dt)
        shm_refs.append(shm_dt)
        shared_dt_arr = np.ndarray(datetime_s.shape, dtype=datetime_s.dtype, buffer=shm_dt.buf)
        shared_dt_arr[:] = datetime_s[:]
        
        shared_info['datetime_sec'] = {
            'type': 'shm',
            'name': shm_dt.name,
            'shape': datetime_s.shape,
            'dtype': str(datetime_s.dtype),
        ...
        # 3. cooling_demand: 存入共享内存（使用 float32 节省内存，如果精度允许）
        cooling_arr = np.array(cooling_demand_list, dtype=np.float32)
        shm_name_cd = f"year{year}_{country_code[:10]}_cd"
        try:
            shm_cd = shared_memory.SharedMemory(create=True, size=cooling_arr.nbytes, name=shm_name_cd)
        except FileExistsError:
            shm_name_cd = f"year{year}_{country_code[:8]}_cd_{uuid.uuid4().hex[:6]}"
            shm_cd = shared_memory.SharedMemory(create=True, size=cooling_arr.nbytes, name=shm_name_cd)
        shm_refs.append(shm_cd)
        shared_cd_arr = np.ndarray(cooling_arr.shape, dtype=cooling_arr.dtype, buffer=shm_cd.buf)
        shared_cd_arr[:] = cooling_arr[:]
        
        shared_info['cooling_demand'] = {
            'type': 'shm',
            'name': shm_cd.name,
        ...
        # 4. case: 转换为整数编码，存入共享内存
        case_int_arr = np.array([case_to_int[case] for case in case_list], dtype=np.int32)
        
        shm_name_case = f"year{year}_{country_code[:10]}_case"
        try:
            shm_case = shared_memory.SharedMemory(create=True, size=case_int_arr.nbytes, name=shm_name_case)
        except FileExistsError:
            shm_name_case = f"year{year}_{country_code[:8]}_case_{uuid.uuid4().hex[:6]}"
            shm_case = shared_memory.SharedMemory(create=True, size=case_int_arr.nbytes, name=shm_name_case)
        shm_refs.append(shm_case)
        shared_case_arr = np.ndarray(case_int_arr.shape, dtype=case_int_arr.dtype, buffer=shm_case.buf)
        shared_case_arr[:] = case_int_arr[:]
        
        shared_info['case'] = {
            'type': 'shm',
            'name': shm_case.name,
            'shape': case_int_arr.shape,
            'dtype': str(case_int_arr.dtype),
            'case_names': case_names_list  # 保存映射关系（全局统一）
        }
        
        shared_info['_shm_refs'] = shm_refs
        country_shared_data[country_code] = shared_info
        
        if country_idx % 10 == 0 or country_idx == total_countries:
            logger.info(f"  已处理 {country_idx}/{total_countries} 个国家")
    
    # 释放记录数据
    del country_records
    gc.collect()
    
    shm_elapsed = time.time() - shm_start
    logger.info(f"步骤4完成，耗时: {shm_elapsed:.2f} 秒，共处理 {len(country_shared_data)} 个国家的数据")
    
    total_elapsed = time.time() - preprocess_start
    logger.info(f"预处理完成，总耗时: {total_elapsed:.2f} 秒")
    
    return country_shared_data
...
def process_single_year(model_name, ssp_path, year):
    ...
    # ========== 第四步：预处理 - 按国家分组并转换为共享内存 ==========
    logger.info("开始预处理：按国家分组并转换为共享内存...")
    preprocess_start = time.time()
    
    # 执行预处理，将数据按国家分组并存入共享内存
    country_shared_data = preprocess_parquet_to_shared_memory(
        model_name, ssp_path, year, point_to_country
    )
    
    preprocess_elapsed = time.time() - preprocess_start
    logger.info(f"预处理完成，耗时: {preprocess_elapsed:.2f} 秒")
    
    # ========== 第五步：并行处理所有国家，从共享内存读取数据 ==========
    logger.info("开始并行处理所有国家的逐时能耗...")
    process_start = time.time()
    
    # 准备所有国家的参数
    country_args = []
    for country_code in country_heat_wave_info_dict.keys():
        if country_code not in country_shared_data:
            continue
        
        continent = country_to_continent.get(country_code, 'Unknown')
        shared_info = country_shared_data[country_code]
        
        country_args.append((
            country_code,
            country_heat_wave_info_dict[country_code],  # 热浪日期-点映射
            continent,
            shared_info,  # 该国家的共享内存信息
            year,
            model_name,
            ssp_path
        ))
    ...
    try:
        with Pool(processes=min(NUM_PROCESSES_COUNTRY, len(country_args))) as pool:
            pool.map(process_single_country_from_shared_memory, country_args)
    finally:
        # 释放所有共享内存
        cleanup_start = time.time()
        logger.info("释放共享内存...")
        for country_code, shared_info in country_shared_data.items():
            if '_shm_refs' in shared_info:
                for shm in shared_info['_shm_refs']:
                    try:
                        shm.close()
                        shm.unlink()
                    except Exception as e:
                        logger.debug(f"清理共享内存 {shm.name} 时出错: {e}")
        cleanup_elapsed = time.time() - cleanup_start
        logger.info(f"共享内存释放完成，耗时: {cleanup_elapsed:.2f} 秒")
    
    process_elapsed = time.time() - process_start
    logger.info(f"所有国家已处理完成 (耗时: {process_elapsed:.2f}s)")
    
    total_elapsed = time.time() - year_start_time
    logger.info(f"年份 {year} 处理完成，总耗时: {total_elapsed:.2f} 秒")
...
def process_multiple_years(model_name, ssp_path, years):
    ...
    for year_idx, year in enumerate(years):
        ...
        batch_case_data = {}
        if parquet_args:
            # 调整参数格式以匹配load_single_parquet_for_preprocess
            parquet_args_simple = [(case_name, parquet_path) for case_name, parquet_path, _, _, _ in parquet_args]
            read_processes = min(len(parquet_args_simple), 7, os.cpu_count())
            with Pool(processes=read_processes) as read_pool:
                read_results = read_pool.map(load_single_parquet_for_preprocess, parquet_args_simple)
            
            for case_name, df_case in read_results:
                if df_case is not None:
                    # 只保留必要列
                    needed_cols = ['point_key', 'datetime', 'cooling_demand', 'case']
                    if all(col in df_case.columns for col in needed_cols):
...
def process_single_country_from_shared_memory(args):
    """并行处理单个国家，从共享内存读取数据
    
    Parameters:
    -----------
    args : tuple
        (country_code, country_heat_wave_dates_dict, continent, shared_info,
         year, model_name, ssp_path)
        country_heat_wave_dates_dict: {date_str: list of (lat, lon) tuples}
        shared_info: 该国家的共享内存信息
    """
    (country_code, country_heat_wave_dates_dict, continent, shared_info,
     year, model_name, ssp_path) = args
    ...
    try:
        ...
        # 读取 lat 和 lon 数组
        lat_info = shared_info['lat']
        existing_shm_lat = shared_memory.SharedMemory(name=lat_info['name'])
        existing_shm_list.append(existing_shm_lat)
        lat_arr = np.ndarray(lat_info['shape'], dtype=lat_info['dtype'], buffer=existing_shm_lat.buf)
        
        lon_info = shared_info['lon']
        existing_shm_lon = shared_memory.SharedMemory(name=lon_info['name'])
        existing_shm_list.append(existing_shm_lon)
        lon_arr = np.ndarray(lon_info['shape'], dtype=lon_info['dtype'], buffer=existing_shm_lon.buf)
        
        # 只在需要时临时构建 point_key（用于匹配）
        point_key_list = list(zip(lat_arr.copy(), lon_arr.copy()))  # 复制避免共享内存问题
        
        # 读取datetime（秒级时间戳）
        dt_info = shared_info['datetime_sec']
        existing_shm_dt = shared_memory.SharedMemory(name=dt_info['name'])
        existing_shm_list.append(existing_shm_dt)
        datetime_sec = np.ndarray(dt_info['shape'], dtype=dt_info['dtype'], buffer=existing_shm_dt.buf)
        datetime_series = pd.to_datetime(datetime_sec, unit='s')
        
        # 读取cooling_demand
        cd_info = shared_info['cooling_demand']
        existing_shm_cd = shared_memory.SharedMemory(name=cd_info['name'])
        existing_shm_list.append(existing_shm_cd)
        cooling_demand = np.ndarray(cd_info['shape'], dtype=cd_info['dtype'], buffer=existing_shm_cd.buf)
        
        # 读取case（整数编码）
        case_info = shared_info['case']
        existing_shm_case = shared_memory.SharedMemory(name=case_info['name'])
        existing_shm_list.append(existing_shm_case)
        case_int = np.ndarray(case_info['shape'], dtype=case_info['dtype'], buffer=existing_shm_case.buf)
        # 将整数编码转换回case名称
        case_names_list = case_info['case_names']
        case_arr = np.array([case_names_list[i] for i in case_int])
        
        # 创建DataFrame
        country_df = pd.DataFrame({
            'point_key': point_key_list,
            'datetime': datetime_series,
            'cooling_demand': cooling_demand.copy(),  # 复制数据，避免共享内存问题
            'case': case_arr
        })
        
        logger.debug(f"  国家 {country_code}: 从共享内存读取了 {len(country_df)} 条记录")
        read_elapsed = time.time() - read_start
        ...
        # 将date_str转换为date对象
        heat_wave_date_point_set = set()
        for date_str, points_list in country_heat_wave_dates_dict.items():
            try:
                if isinstance(date_str, str):
                    date = pd.to_datetime(date_str).date()
                else:
        ...
        # Step 3: 过滤出热浪日期+热浪点的数据
        filter_start = time.time()
        # 创建date列（从datetime提取）
        country_df['date'] = pd.to_datetime(country_df['datetime']).dt.date
        
        # 创建(date, point_key)的组合用于过滤
        country_df['date_point'] = list(zip(country_df['date'], country_df['point_key']))
        mask = country_df['date_point'].isin(heat_wave_date_point_set)
        filtered_df = country_df[mask].copy()
        
        # 删除临时列
        del country_df
        del filtered_df['date_point']
        gc.collect()
        
        filter_elapsed = time.time() - filter_start
        
        if len(filtered_df) == 0:
            logger.debug(f"  国家 {country_code}: 无热浪数据")
            # 关闭共享内存引用
        ...
        # Step 4: 按datetime和case分组，累加cooling_demand
        calc_start = time.time()
        # 按datetime和case分组累加
        result_df = filtered_df.groupby(['datetime', 'case'])['cooling_demand'].sum().reset_index()
        
        # 转换为透视表格式：datetime作为索引，case作为列
        result_pivot = result_df.pivot(index='datetime', columns='case', values='cooling_demand').fillna(0.0)
        ...
        for dt in time_range:
            row_data = {'time': dt}
            # 从透视表中获取该时间点的所有工况能耗
            if dt in result_pivot.index:
                for case_name in CASES:
                    if case_name in result_pivot.columns:
                        row_data[case_name] = float(result_pivot.loc[dt, case_name])
                    else:
                        row_data[case_name] = 0.0
            else:
                # 该时间点没有热浪数据，所有工况能耗为0
                for case_name in CASES:
                    row_data[case_name] = 0.0
        ...
        output_file = os.path.join(output_dir, f"{country_code}_{year}_all_cases_hourly_energy.csv")
        final_df.to_csv(output_file, index=False)
        
        save_elapsed = time.time() - save_start
        total_elapsed = time.time() - total_start_time
        
        logger.info(f"  ✓ {country_code}: 已保存 (读取: {read_elapsed:.2f}s, 准备: {prep_elapsed:.2f}s, "
                   f"过滤: {filter_elapsed:.2f}s, 计算: {calc_elapsed:.2f}s, 保存: {save_elapsed:.2f}s, "
...
def process_single_country_batch_cases(args):
    ...
    try:
        ...
        if batch_results:
            ...
            # 文件名：如果包含所有21个工况，使用all_cases；否则使用批次范围
            if len(case_names) == 21:  # 21个工况：ref + case1-20
                batch_name = "all_cases"
            else:
...
def cleanup_orphaned_shared_memory():
    """清理残留的共享内存（程序异常退出时可能残留）"""
    try:
        shm_dir = '/dev/shm'
        if not os.path.exists(shm_dir):
            # Windows 系统可能没有 /dev/shm，跳过清理
            return
        
        cleaned_count = 0
        for name in os.listdir(shm_dir):
            # 清理以 'year' 或 'batch' 开头的共享内存
            if name.startswith('year') or name.startswith('batch'):
                try:
                    shm = shared_memory.SharedMemory(name=name)
                    shm.close()
                    shm.unlink()
                    cleaned_count += 1
                    logger.info(f"清理残留共享内存: {name}")
                except Exception as e:
                    # 忽略错误（可能已被清理或权限问题）
                    pass
        
        if cleaned_count > 0:
            logger.info(f"共清理 {cleaned_count} 个残留的共享内存段")
    except Exception as e:
        logger.debug(f"清理共享内存时出错（可忽略）: {e}")
