    ...
    try:
        ...
        for model_name in MODELS:
            for ssp_path in SSP_PATHS:
                # 使用方案B：同时处理所有年份（分批读取，并行处理）
                try:
                    process_multiple_years(model_name, ssp_path, TARGET_YEARS)
                    
                    # 每个SSP处理完后，再次确保内存清理
                    logger.info(f"\n{model_name} - {ssp_path} 处理完成，执行最终内存清理...")
                    for i in range(2):
                        collected = gc.collect()
                        if i == 0:
                            logger.info(f"  最终GC: 回收了 {collected} 个对象")
                    logger.info(f"{'='*80}\n")
                    
                except Exception as e:
                    logger.error(f"处理 {model_name} - {ssp_path} - {TARGET_YEARS} 年时出错: {e}")
                    import traceback
                    traceback.print_exc()
                    # 即使出错也要清理内存
                    for i in range(2):
