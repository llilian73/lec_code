...
# 模型配置
MODELS = ["EC-Earth3"]  # 可以修改为其他模型

# SSP配置
SSP_PATHS = ["SSP126"]  # 可以修改为其他SSP

# 年份配置（支持同时处理多年份）
TARGET_YEARS = [2033, 2034]  # 5年数据
...
# 并行处理配置
NUM_PROCESSES_COUNTRY = 7  # 计算国家逐时能耗的进程数（一个进程计算一个国家）

# 象限划分配置
QUADRANT_COUNT = 4  # 4个象限
...
def get_quadrant_name(quadrant):
    """获取象限名称"""
    names = {
        0: "东北象限 (lat>=0, lon>=0)",
        1: "西北象限 (lat>=0, lon<0)",
    ...
    return names.get(quadrant, f"未知象限 {quadrant}")


def filter_data_by_quadrant(df, quadrant):
    """根据象限过滤DataFrame中的数据点
    
    Parameters:
    -----------
    df : pd.DataFrame
        包含lat和lon列的DataFrame
    quadrant : int
        象限编号（0-3）
    ...
    if quadrant == 0:  # lat >= 0, lon >= 0
        mask = (df['lat'] >= 0) & (df['lon'] >= 0)
    elif quadrant == 1:  # lat >= 0, lon < 0
        mask = (df['lat'] >= 0) & (df['lon'] < 0)
    elif quadrant == 2:  # lat < 0, lon >= 0
        mask = (df['lat'] < 0) & (df['lon'] >= 0)
    elif quadrant == 3:  # lat < 0, lon < 0
        mask = (df['lat'] < 0) & (df['lon'] < 0)
    else:
        return df
...
def filter_points_by_quadrant(points_dict, quadrant):
    """根据象限过滤点字典
    
    Parameters:
    -----------
    points_dict : dict
        点的字典，key可以是 (lat, lon) 或其他格式
    quadrant : int
        象限编号（0-3）
    ...
    for key, value in points_dict.items():
        if isinstance(key, tuple) and len(key) == 2:
            lat, lon = key
            q = get_quadrant(lat, lon)
            if q == quadrant:
                filtered[key] = value
        elif isinstance(key, (list, tuple)) and len(key) >= 2:
            # 如果key是列表或元组，尝试提取lat和lon
            lat, lon = key[0], key[1]
            q = get_quadrant(lat, lon)
            if q == quadrant:
                filtered[key] = value
...
def load_single_parquet_quadrant(args):
    """并行读取单个parquet文件，只读取指定象限的数据
    
    Parameters:
    -----------
    args : tuple
        (case_name, parquet_path, model_name, ssp_path, year, quadrant)
    
    Returns:
    --------
    tuple: (case_name, df_hourly) 或 (case_name, None) 如果失败
    """
    case_name, parquet_path, model_name, ssp_path, year, quadrant = args
    ...
    try:
        ...
        # 按象限过滤数据
        df_hourly = filter_data_by_quadrant(df_hourly, quadrant)
        
        if df_hourly.empty:
            return (case_name, None)
        
        # 合并date和time列创建datetime（与原来的逻辑相同）
...
def process_single_year(model_name, ssp_path, year):
    ...
    # ========== 第四步：建立国家-点的完整映射（用于跟踪哪些点已处理）==========
    t0 = time.time()
    # 格式：{country_code: set of (lat, lon) tuples} - 每个国家有热浪数据的点（只跟踪需要处理的点）
    country_all_points = {}
    for point_key, heat_wave_dates in point_to_heat_wave_dates.items():
        if not heat_wave_dates:  # 如果没有热浪数据，跳过
            continue
        country_code, continent = point_to_country.get(point_key, (None, None))
        if country_code is None:
            continue
        if country_code not in country_all_points:
            country_all_points[country_code] = set()
        country_all_points[country_code].add(point_key)
    
    logger.info(f"[时间统计] 建立国家-点映射总耗时: {time.time() - t0:.2f} 秒")
    logger.info(f"  共 {len(country_all_points)} 个国家，总点数: {sum(len(points) for points in country_all_points.values())}")
    
    # ========== 第五步：按象限处理（一次只处理一个象限，一次处理所有21个工况）==========
    total_start_time = time.time()
    
    # 用于累积跨象限国家的数据
    # 格式：{country_code: {case_name: {datetime_str: energy}}}
    accumulated_country_results = {}
    
    # 用于跟踪每个国家已处理的点
    # 格式：{country_code: set of (lat, lon) tuples}
    country_processed_points = {}
    ...
    for quadrant in range(4):
        quadrant_name = get_quadrant_name(quadrant)
        logger.info(f"\n{'='*80}")
        logger.info(f"处理 {quadrant_name}")
        logger.info(f"{'='*80}")
        
        quadrant_start_time = time.time()
        
        # ========== 第一步：读取所有21个工况的parquet，但只读取当前象限的数据 ==========
        logger.info(f"读取 {quadrant_name} 的所有21个工况数据...")
        read_start_time = time.time()
        
        # 并行读取所有21个工况的parquet文件
        parquet_args = []
        for case_name in CASES:
            ...
            if os.path.exists(parquet_path):
                parquet_args.append((case_name, parquet_path, model_name, ssp_path, year, quadrant))
        
        # 并行读取（限制进程数）
        quadrant_case_data = {}  # {case_name: df} - 只包含当前象限的数据
        if parquet_args:
            read_processes = min(7, os.cpu_count())
            with Pool(processes=read_processes) as read_pool:
                read_results = read_pool.map(load_single_parquet_quadrant, parquet_args)
            
            for case_name, df_case in read_results:
                if df_case is not None and not df_case.empty:
                    quadrant_case_data[case_name] = df_case
                    logger.info(f"  {case_name}: 读取了 {len(df_case)} 条记录（{quadrant_name}）")
            
            del read_results
            gc.collect()
        
        read_elapsed = time.time() - read_start_time
        logger.info(f"  {quadrant_name}: 读取完成，成功加载 {len(quadrant_case_data)} 个工况 (耗时: {read_elapsed:.2f}s)")
        
        if not quadrant_case_data:
            logger.warning(f"{quadrant_name}没有有效的工况数据，跳过")
            processed_quadrants.add(quadrant)
            continue
        
        # ========== 第二步：合并所有21个工况的数据 ==========
        logger.info(f"合并 {quadrant_name} 的所有工况数据...")
        merge_start_time = time.time()
        
        # 使用第一个工况的point_key和datetime作为基础
        first_case = list(quadrant_case_data.keys())[0]
        first_df = quadrant_case_data[first_case]
        ...
        # 为每个工况存储cooling_demand列
        for case_name, df_case in quadrant_case_data.items():
            if 'cooling_demand' in df_case.columns:
                cooling_arr = df_case['cooling_demand'].values
                shm_name = f"quadrant{quadrant}_{case_name[:10]}_cd"
                try:
                    shm = shared_memory.SharedMemory(create=True, size=cooling_arr.nbytes, name=shm_name)
                except FileExistsError:
                    shm_name = f"quadrant{quadrant}_{case_name[:8]}_cd_{uuid.uuid4().hex[:6]}"
                    shm = shared_memory.SharedMemory(create=True, size=cooling_arr.nbytes, name=shm_name)
                shared_arr = np.ndarray(cooling_arr.shape, dtype=cooling_arr.dtype, buffer=shm.buf)
                shared_arr[:] = cooling_arr[:]
                
                quadrant_shared_data['_case_data'][case_name] = {
                    'type': 'shm',
                    'name': shm.name,
                    'shape': cooling_arr.shape,
                    'dtype': str(cooling_arr.dtype)
                }
                quadrant_shared_data['_shm_refs'].append(shm)
        
        # 释放原始DataFrame
        del quadrant_case_data
        gc.collect()
        
        merge_elapsed = time.time() - merge_start_time
        logger.info(f"  {quadrant_name}: 转换为共享内存格式完成 (耗时: {merge_elapsed:.2f}s)")
        
        # ========== 第三步：按国家聚合当前象限的数据 ==========
        logger.info(f"按国家聚合 {quadrant_name} 的数据...")
        aggregate_start_time = time.time()
        
        # 为该象限创建国家-热浪日期-点映射（只包含当前象限的点）
        quadrant_country_info = {}  # {country_code: {date_str: list of points}}
        for (lat, lon), (country_code, continent) in point_to_country.items():
            if country_code is None:
                continue
            # 检查该点是否属于当前象限
            if get_quadrant(lat, lon) != quadrant:
                continue
            
            point_key = (lat, lon)
            heat_wave_dates = point_to_heat_wave_dates.get(point_key, set())
            
            if country_code not in quadrant_country_info:
                quadrant_country_info[country_code] = {}
            
            for date in heat_wave_dates:
                date_str = date.isoformat() if isinstance(date, datetime) else str(date)
                if date_str not in quadrant_country_info[country_code]:
                    quadrant_country_info[country_code][date_str] = []
                quadrant_country_info[country_code][date_str].append(point_key)
        
        # 准备所有国家的参数
        country_args = []
        for country_code in quadrant_country_info.keys():
            continent = country_to_continent.get(country_code, 'Unknown')
            country_args.append((
                country_code,
                quadrant_country_info[country_code],  # 当前象限的热浪数据
                continent,
                CASES,  # 所有21个工况
                quadrant_shared_data,  # 共享内存数据
                year,
                model_name,
                ssp_path,
                quadrant
        ...
        # 并行处理所有国家（当前象限）
        logger.info(f"  并行处理 {len(country_args)} 个国家的 {quadrant_name} 数据（{NUM_PROCESSES_COUNTRY}个进程）...")
        
        pool_start = time.time()
        with Pool(processes=min(NUM_PROCESSES_COUNTRY, len(country_args))) as pool:
            quadrant_results = pool.starmap(process_country_quadrant_data, country_args)
        pool_elapsed = time.time() - pool_start
        logger.info(f"  所有国家处理完成 (耗时: {pool_elapsed:.2f}s)")
        
        # ========== 第四步：累积跨象限国家的数据 ==========
        logger.info(f"累积跨象限国家的数据...")
        accumulate_start_time = time.time()
        
        for country_code, case_results in quadrant_results:
            if country_code is None:
                continue
            
            # 初始化累积结果（如果还没有）
            if country_code not in accumulated_country_results:
                accumulated_country_results[country_code] = {}
                country_processed_points[country_code] = set()
                for case_name in CASES:
                    accumulated_country_results[country_code][case_name] = {}
            
            # 累加当前象限的结果
            for case_name, energy_dict in case_results.items():
                for datetime_str, energy in energy_dict.items():
                    if datetime_str in accumulated_country_results[country_code][case_name]:
                        accumulated_country_results[country_code][case_name][datetime_str] += energy
                    else:
                        accumulated_country_results[country_code][case_name][datetime_str] = energy
            
            # 更新已处理的点（从当前象限的国家映射中获取）
            if country_code in quadrant_country_info:
                for points_list in quadrant_country_info[country_code].values():
                    country_processed_points[country_code].update(points_list)
        
        accumulate_elapsed = time.time() - accumulate_start_time
        logger.info(f"  累积完成 (耗时: {accumulate_elapsed:.2f}s)")
        
        # ========== 第五步：检查并保存已完成的国家 ==========
        logger.info(f"检查已完成的国家...")
        ...
        # 检查哪些国家的所有点都已处理完
        completed_countries = []
        for country_code in list(accumulated_country_results.keys()):
            if country_code in country_all_points and country_code in country_processed_points:
                all_points = country_all_points[country_code]
                processed_points = country_processed_points[country_code]
                
                # 检查是否所有点都已处理
                if all_points.issubset(processed_points):
                    completed_countries.append(country_code)
        
        if completed_countries:
            logger.info(f"  发现 {len(completed_countries)} 个已完成的国家，开始保存...")
            
            # 保存已完成国家的结果
            for country_code in completed_countries:
                continent = country_to_continent.get(country_code, 'Unknown')
                save_country_results(
                    country_code, continent, accumulated_country_results[country_code],
                    year, model_name, ssp_path
                )
            
            # 从累积结果中移除已完成的国家
            for country_code in completed_countries:
                del accumulated_country_results[country_code]
                del country_processed_points[country_code]
            
            logger.info(f"  {len(completed_countries)} 个国家的数据已保存并释放内存")
        
        check_elapsed = time.time() - check_start_time
        logger.info(f"  检查完成 (耗时: {check_elapsed:.2f}s)")
        
        # 释放象限共享内存
        cleanup_start = time.time()
        if '_shm_refs' in quadrant_shared_data:
            for shm in quadrant_shared_data['_shm_refs']:
                try:
                    shm.close()
                    shm.unlink()
                except Exception as e:
                    logger.debug(f"清理 shm {shm.name} 时出错: {e}")
        del quadrant_shared_data
        gc.collect()
        
        cleanup_elapsed = time.time() - cleanup_start
        logger.info(f"  共享内存释放完成 (耗时: {cleanup_elapsed:.2f}s)")
        
        quadrant_elapsed = time.time() - quadrant_start_time
        logger.info(f"{quadrant_name}处理完成，耗时: {quadrant_elapsed:.2f} 秒")
    
    # ========== 第六步：保存剩余未完成的国家（如果有）==========
    if accumulated_country_results:
        logger.warning(f"还有 {len(accumulated_country_results)} 个国家的数据未完成，可能是数据不完整")
        # 可以选择保存这些部分数据，或者记录警告
        for country_code in list(accumulated_country_results.keys()):
            continent = country_to_continent.get(country_code, 'Unknown')
            save_country_results(
                country_code, continent, accumulated_country_results[country_code],
                year, model_name, ssp_path, suffix="_partial"
            )
    
    logger.info(f"\n所有象限处理完成，总耗时: {time.time() - total_start_time:.2f} 秒")
    
    # ========== 第七步：合并所有国家的象限和批次文件 ==========
    logger.info(f"\n{'='*80}")
    logger.info("合并所有国家的象限和批次文件...")
    logger.info(f"{'='*80}")
    merge_start_time = time.time()
    
    # 获取所有有热浪数据的国家
    all_countries = set()
    for quadrant_info in quadrant_country_info.values():
        all_countries.update(quadrant_info.keys())
    
    # 并行合并所有国家的文件
    merge_args = []
    for country_code in all_countries:
        continent = country_to_continent.get(country_code, 'Unknown')
        merge_args.append((model_name, ssp_path, year, country_code, continent))
    
    # 并行合并（限制进程数）
    merge_processes = min(4, len(merge_args), os.cpu_count())
    if merge_args:
        with Pool(processes=merge_processes) as merge_pool:
            merge_pool.starmap(merge_country_quadrant_files, merge_args)
    
    merge_elapsed = time.time() - merge_start_time
    logger.info(f"所有国家文件合并完成，耗时: {merge_elapsed:.2f} 秒")
...
def merge_country_quadrant_files(model_name, ssp_path, year, country_code, continent):
    """合并同一国家的所有象限和批次文件到一个文件
    ...
    try:
        ...
        # 查找所有象限和批次文件
        quadrant_batch_files = {}  # {(quadrant, batch_name): file_path}
        
        for quadrant in range(4):
            for batch_idx in range(1, 10):  # 最多9个批次（实际可能更少）
                batch_name = f"batch{batch_idx}"
                file_pattern = f"{country_code}_{year}_q{quadrant}_{batch_name}_hourly_energy.csv"
                file_path = os.path.join(output_dir, file_pattern)
                
                if os.path.exists(file_path):
                    quadrant_batch_files[(quadrant, batch_name)] = file_path
        
        if not quadrant_batch_files:
            logger.debug(f"  国家 {country_code}: 没有找到象限批次文件，跳过合并")
            return
        
        # 读取所有文件并合并
        all_dataframes = []
        for (quadrant, batch_name), file_path in quadrant_batch_files.items():
            try:
        ...
        # 收集所有工况的数据（累加同一工况，合并不同工况）
        case_energy_dict = {}  # {case_name: {time_str: energy}}
        ...
        for df in all_dataframes:
            ...
            if 'time' in df.columns:
                ...
                for col in df.columns:
                    ...
                    # col是工况名称（如 'ref', 'case1' 等）
                    if col not in case_energy_dict:
                        case_energy_dict[col] = {}
                    
                    # 累加该工况的能耗（按时间点）
                    for idx, row in df.iterrows():
                        time_str = row['time'].isoformat()
                        energy = row[col]
                        if time_str in case_energy_dict[col]:
                            case_energy_dict[col][time_str] += energy
                        else:
                            case_energy_dict[col][time_str] = energy
        
        # 构建最终的DataFrame（按照CASES的顺序排列工况列）
        result_data = []
        for dt in time_range:
            ...
            # 按照CASES的顺序添加所有工况的能耗
            for case_name in CASES:
                if case_name in case_energy_dict:
                    energy = case_energy_dict[case_name].get(dt_str, 0.0)
                else:
                    energy = 0.0
        ...
        logger.info(f"  ✓ {country_code}: 合并完成，已保存到 {output_file}")
        
        # 可选：删除象限批次文件（节省空间）
        # for file_path in quadrant_batch_files.values():
        #     try:
        #         os.remove(file_path)
        #     except:
        #         pass
        ...
        logger.error(f"合并国家 {country_code} 的文件时出错: {e}")
        import traceback
        traceback.print_exc()


def process_country_quadrant_data(country_code, country_heat_wave_dates_dict, continent, 
                                   case_names, quadrant_shared_data, year, model_name, ssp_path, quadrant):
    """处理单个国家在当前象限的数据
    ...
    country_code : str
        国家代码
    country_heat_wave_dates_dict : dict
        当前象限的国家-热浪日期-点映射 {date_str: list of points}
    continent : str
        大洲名称
    case_names : list
        工况列表（所有21个工况）
    quadrant_shared_data : dict
        象限共享数据
    year : int
        年份
    model_name : str
    ...
    Returns:
    --------
    tuple: (country_code, {case_name: {datetime_str: energy}})
    """
    existing_shm_dt = None
    existing_shm_list = []
    
    try:
        # Step 1: 收集该国家当前象限的所有热浪点 + 日期
        all_points = set()
        date_point_set = set()
        
        for date_str, points in country_heat_wave_dates_dict.items():
            try:
        ...
        global_point_key = quadrant_shared_data['_point_key']
        point_key_to_indices = quadrant_shared_data['_point_key_to_indices']
        
        # 从共享内存读取datetime
        dt_info = quadrant_shared_data['_datetime_sec']
        ...
        country_results = {}  # {case_name: {datetime_str: energy}}
        
        for case_name in case_names:
            if case_name not in quadrant_shared_data.get('_case_data', {}):
                continue
            
            case_info = quadrant_shared_data['_case_data'][case_name]
            existing_shm = shared_memory.SharedMemory(name=case_info['name'])
            existing_shm_list.append(existing_shm)
        ...
        existing_shm_dt.close()
        return (country_code, country_results)
    
    except Exception as e:
        logger.error(f"处理国家 {country_code} - 象限 {quadrant} 时出错: {e}")
        import traceback
        traceback.print_exc()
        return (country_code, {})
    finally:
...
def save_country_results(country_code, continent, country_results, year, model_name, ssp_path, suffix=""):
    """保存国家结果到CSV文件
    ...
    try:
        ...
        output_file = os.path.join(output_dir, f"{country_code}_{year}_hourly_energy{suffix}.csv")
        result_df.to_csv(output_file, index=False)
        
        logger.info(f"  ✓ {country_code}: 已保存到 {output_file}")
    
    except Exception as e:
        logger.error(f"保存国家 {country_code} 的结果时出错: {e}")
        import traceback
        traceback.print_exc()
...
def process_single_country_batch_cases(args):
    ...
    args : tuple
        (country_code, shared_country_heat_wave_dates_dict, continent, case_names, batch_shared_data,
         year, model_name, ssp_path, quadrant)
    """
    (country_code, shared_country_heat_wave_dates_dict, continent, case_names, batch_shared_data,
     year, model_name, ssp_path, quadrant) = args
    ...
    try:
        ...
        if batch_results:
            ...
            # 文件名：根据批次和象限生成文件名
            # 第一次批次（7个）：ref + case1-6 -> batch1
            # 第二次批次（7个）：case7-13 -> batch2
            # 第三次批次（7个）：case14-20 -> batch3
            if len(case_names) == 7:
                if case_names[0] == 'ref' and case_names[-1] == 'case6':
                    batch_name = "batch1"  # 第一批：ref + case1-6
                elif case_names[0] == 'case7' and case_names[-1] == 'case13':
                    batch_name = "batch2"  # 第二批：case7-13
                elif case_names[0] == 'case14' and case_names[-1] == 'case20':
                    batch_name = "batch3"  # 第三批：case14-20
                else:
            ...
            # 添加象限标识到文件名
            quadrant_name = f"q{quadrant}"
            output_file = os.path.join(output_dir, f"{country_code}_{year}_{quadrant_name}_{batch_name}_hourly_energy.csv")
            result_df.to_csv(output_file, index=False)
