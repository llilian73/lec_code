...
def load_single_parquet(args):
    ...
    try:
        ...
        if 'datetime' in df_hourly.columns:
            ...
            # 只保留必要列：lat, lon, cooling_demand, point_key, datetime_str
...
def process_single_year(model_name, ssp_path, year):
    ...
    # ========== 第五步：按批次处理工况（一次加载所有21个工况），并行计算国家逐时能耗，立即输出 ==========
    total_start_time = time.time()
    
    # 批次大小：一次加载所有工况（21个）
    BATCH_SIZE = len(CASES)  # 21个工况
    
    # 将工况分成批次（实际上只有一个批次，包含所有21个工况）
    case_batches = [CASES[i:i+BATCH_SIZE] for i in range(0, len(CASES), BATCH_SIZE)]
    
    # 按批次处理
    for batch_idx, case_batch in enumerate(case_batches):
        batch_start_time = time.time()
        logger.info(f"\n{'='*80}")
        logger.info(f"处理批次 {batch_idx + 1}/{len(case_batches)}: {', '.join(case_batch)}")
        logger.info(f"{'='*80}")
        ...
        # 并行读取parquet文件（一次读取所有21个工况）
        batch_case_data = {}  # {case_name: df}
        if parquet_args:
            # 并行读取进程数：使用21或CPU核心数，取较小值
            read_processes = min(len(parquet_args), 21, os.cpu_count())
        ...
        logger.info(f"  并行计算完成 (耗时: {pool_elapsed:.2f}s)")
        ...
        logger.info(f"  批次 {batch_idx + 1} 处理完成，耗时: {batch_elapsed:.2f} 秒")
...
def process_multiple_years_batch(batch_items):
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"处理批次: {model_name} - {len(batch_items)} 个年份")
    for ssp_path, year in years_info:
        logger.info(f"  - {ssp_path} - {year}")
    logger.info(f"{'='*80}")
...
def process_multiple_years(model_name, ssp_path, years):
    ...
    total_start_time = time.time()
    logger.info(f"\n{'='*80}")
    logger.info(f"处理 {model_name} - {ssp_path} - {len(years)} 个年份: {years}")
    logger.info(f"{'='*80}")
    ...
    logger.info(f"\n{'='*80}")
    logger.info("开始分批读取parquet文件（方案B：分批读取，并行处理）")
    logger.info(f"{'='*80}")
    ...
    # ========== 第五步：并行处理所有国家（所有年份）==========
    logger.info(f"\n{'='*80}")
    logger.info("开始并行处理所有国家的逐时能耗（所有年份）")
    logger.info(f"{'='*80}")
    ...
    for i in range(3):
        ...
        if i == 0:
            logger.info(f"  第1次GC: 回收了 {collected} 个对象")
    
    cleanup_elapsed = time.time() - cleanup_start
    logger.info(f"内存释放完成 (耗时: {cleanup_elapsed:.2f}s)")
    
    total_elapsed = time.time() - total_start_time
    logger.info(f"\n[时间统计] 所有年份处理总耗时: {total_elapsed:.2f} 秒")
    logger.info(f"{'='*80}\n")
    ...
    try:
        ...
        if country_case_energy:
            ...
            logger.info(f"  ✓ {country_code} - {case_name}: 已保存 ({len(country_case_energy)} 个时间点) | "
                       f"准备: {prep_elapsed:.2f}s, 读取: {read_elapsed:.2f}s, 计算: {calc_elapsed:.2f}s, 保存: {save_elapsed:.2f}s, 总计: {total_elapsed:.2f}s")
...
def process_single_country_batch_cases(args):
    ...
    try:
        ...
        if batch_results:
            ...
            # 精简日志：每个国家只打印一次成功信息
            logger.info(f"  ✓ {country_code} - 批次({len(case_names)}个工况): 已保存 ({total_time_points} 个时间点) | "
                       f"准备: {prep_elapsed:.2f}s, 读取: {read_elapsed:.2f}s, 计算: {calc_elapsed:.2f}s, 保存: {save_elapsed:.2f}s, 总计: {total_elapsed:.2f}s")
    ...
    try:
        ...
        for year in years:
            ...
            if batch_results:
                ...
                logger.info(f"  ✓ {country_code} - {year} ({len(case_names)}个工况): 已保存 ({total_time_points} 个时间点) | "
                           f"准备: {prep_elapsed:.2f}s, 读取: {read_elapsed:.2f}s, 计算: {calc_elapsed:.2f}s, 保存: {save_elapsed:.2f}s, 总计: {year_elapsed:.2f}s")
...
def main():
    ...
    try:
        ...
        logger.info(f"批次大小: {BATCH_YEARS} 年（{BATCH_YEARS * len(CASES)} 个工况）")
        
        # 生成批次列表
        batches = generate_batches()
        logger.info(f"共 {len(batches)} 个批次需要处理")
        ...
        for batch_idx, batch_items in enumerate(batches, 1):
            ...
            try:
                process_multiple_years_batch(batch_items)
                
                # 每个批次处理完后，确保内存清理
                logger.info(f"\n批次 {batch_idx} 处理完成，执行内存清理...")
                for i in range(2):
                    collected = gc.collect()
                    if i == 0:
                        logger.info(f"  批次GC: 回收了 {collected} 个对象")
                logger.info(f"{'='*80}\n")
