...
3. 处理未来数据：提取2056-2060年数据，每个变量一个文件
"""
...
# ============================================================================
# 所有可用的模型列表
ALL_MODELS = ["BCC-CSM2-MR", "EC-Earth3", "ACCESS-ESM1-5", "CanESM5", "MPI-ESM1-2-HR", "MRI-ESM2-0"]
# "ACCESS-ESM1-5", "CanESM5", "MPI-ESM1-2-HR", "MRI-ESM2-0"
# 已处理的模型列表（这些模型将被跳过）
PROCESSED_MODELS = []  # 如果模型已处理完成，添加到这个列表

# 是否处理所有模型（True）或只处理单个模型（False）
PROCESS_ALL_MODELS = True  # 设置为True以处理所有模型
MODEL_NAME = "ACCESS-ESM1-5"  # 当PROCESS_ALL_MODELS=False时使用此模型
...
EC_EARTH_HISTORICAL_DIR = r"Z:\CMIP6\tasmax"
EC_EARTH_FUTURE_DIR = r"Z:\local_environment_creation\heat_wave\GCM_input\EC-Earth3\new"  # EC-Earth3未来数据特殊路径
OUTPUT_BASE_DIR = r"Z:\local_environment_creation\heat_wave\GCM_input_filter"

# 时间范围配置
HISTORICAL_YEARS = (1981, 2010)  # 历史数据提取年份
FUTURE_YEARS = (2051, 2055)  # 未来数据提取年份

# 未来数据变量列表
FUTURE_VARIABLES = ['tas', 'tasmax', 'vas', 'uas', 'huss', 'rsds']
# 'tas', 'tasmax', 'vas', 'uas', 'huss', 'rsds'
# 未来数据情景列表
...
def find_future_files(input_base_dir, model_name, variables, scenarios, start_year, end_year):
    ...
    if model_name == "ACCESS-ESM1-5":
        ...
        # 查找两个文件：20550101-20591231 和 20600101-20641231
        search_dirs = [
            os.path.join(input_base_dir, model_name),
            input_base_dir
        ]
        target_time_ranges = ["20550101-20591231", "20600101-20641231"]
            ...
            for root, dirs, filenames in os.walk(search_dir):
                for filename in filenames:
                    ...
                    for target_time_range in target_time_ranges:
                        if target_time_range in filename:
                            filepath = os.path.join(root, filename)
                            try:
                                file_info = parse_filename(filepath)
                                if (file_info['model'] == model_name and
        ...
        # EC-Earth3在指定文件夹下，一年一个文件，需要查找所有相关年份的文件
        search_dir = EC_EARTH_FUTURE_DIR
        if not os.path.exists(search_dir):
...
def get_future_folder_name(start_year, end_year):
    """根据年份范围生成未来数据文件夹名称
    
    例如：
        2051-2055 -> future_51_55
        2046-2050 -> future_46_50
        2056-2060 -> future_56_60
    """
    start_suffix = str(start_year)[-2:]  # 获取年份后两位
    end_suffix = str(end_year)[-2:]
    return f"future_{start_suffix}_{end_suffix}"
...
def process_future_data(model_name, input_base_dir, output_dir, variables, scenarios, start_year, end_year):
    ...
    for (variable, scenario), group_files in file_groups.items():
        ...
        try:
            ...
            future_folder_name = get_future_folder_name(start_year, end_year)
            output_path = os.path.join(output_dir, model_name, future_folder_name, scenario_upper)
            os.makedirs(output_path, exist_ok=True)
...
def main():
    """主函数"""
    print(f"CMIP6数据年份筛选工具 - 提取2056-2060年未来数据")
    ...
    if PROCESS_ALL_MODELS:
        ...
        print(f"\n将处理以下模型: {', '.join(models_to_process)}")
    else:
