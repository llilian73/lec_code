...
# 配置参数
CLIMATE_DATA_DIR_SLV = r"Z:\local_environment_creation\energy_consumption_gird\weather\2019\M2T1NXSLV"
CLIMATE_DATA_DIR_RAD = r"Z:\local_environment_creation\energy_consumption_gird\weather\2019\M2T1NXRAD"
...
OUTPUT_DIR = r"Z:\local_environment_creation\energy_consumption_gird\result\data"
...
def load_population_data():
    ...
    # 保存到CSV
    population_csv_path = os.path.join(OUTPUT_DIR, 'population_points.csv')
...
def generate_climate_filepaths():
    """生成2019年气候数据文件路径"""
    logging.info("生成气候数据文件路径...")

    slv_files = []
    rad_files = []
    missing_dates = []

    for dt in pd.date_range('2019-01-01', '2019-12-31', freq='D'):
        ...
        slv_path = os.path.join(CLIMATE_DATA_DIR_SLV, slv_fname)
        rad_path = os.path.join(CLIMATE_DATA_DIR_RAD, rad_fname)
    ...
    logging.info(f"找到 {len(slv_files)} 个气候数据文件对")
    if missing_dates:
        logging.warning(f"缺失的数据日期: {missing_dates}")
        logging.warning(f"总共缺失 {len(missing_dates)} 天的数据")
    else:
        logging.info("所有气候数据文件都存在")

    return slv_files, rad_files


def extract_climate_data_for_point(point_data, slv_files, rad_files):
    """为单个点提取气象数据"""
    try:
...
def write_point_data_batch(points_data_batch):
    """并行写入一批点的数据到CSV文件"""
    try:
        results = []
...
def append_point_data_batch(points_data_batch):
    """并行追加一批点的数据到现有CSV文件"""
    try:
        results = []
...
def process_climate_batch(batch_data, slv_files, rad_files):
    """处理一批点的气象数据"""
    try:
        batch_results = []

        for _, point_data in batch_data.iterrows():
            result = extract_climate_data_for_point(point_data, slv_files, rad_files)
...
def extract_climate_data_for_all_points(population_df, slv_files, rad_files):
    """为所有有效人口点提取气象数据 - 超优化版本"""
    logging.info("开始为所有有效人口点提取气象数据...")
    log_memory_usage()
    ...
    for batch_idx, (slv_batch, rad_batch) in enumerate(file_batches):
        ...
        with multiprocessing.Pool(processes=num_processes) as pool:
            # 第一批数据直接写入，后续批次追加写入
            if batch_idx == 0:
                write_func = write_point_data_batch
            else:
                write_func = append_point_data_batch
    ...
    try:
        if 'failed_points' in locals() and failed_points:
            failed_points_path = os.path.join(OUTPUT_DIR, 'failed_climate_points.csv')
...
def main():
    ...
    try:
        ...
        # 第二步：生成气候数据文件路径
        logging.info("=== 第二步：生成气候数据文件路径 ===")
        slv_files, rad_files = generate_climate_filepaths()

        # 第三步：为所有有效人口点提取气象数据
        logging.info("=== 第三步：提取气象数据 ===")
        extract_climate_data_for_all_points(population_df, slv_files, rad_files)
