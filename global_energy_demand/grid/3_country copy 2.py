...
def load_grid_point_results(point_to_country=None):
    """加载网格点结果数据（优化版本：只加载有国家映射的点）"""
    ...
    for file in result_files:
        ...
        if 'point_lat' in coord_part and '_lon' in coord_part:
            try:
                ...
                # 如果提供了点-国家映射，只保留有映射的点
                if point_to_country is not None:
                    if (lat, lon) in point_to_country:
                        valid_points.add((lat, lon))
    ...
    # 如果提供了映射，只返回有效的点
    if point_to_country is not None:
        logging.info(f"找到 {len(point_coords)} 个网格点，其中 {len(valid_points)} 个有国家映射")
...
def process_point_batch(point_batch, point_to_country):
    """处理一批网格点的能耗数据（使用点-国家映射，优化版本）"""
    batch_results = {}
    ...
    for lat, lon in point_batch:
        ...
        if country_iso is not None:
            ...
            if country_iso == 'CN-TW':
                country_iso = 'CN'
            valid_points.append((lat, lon, country_iso))

    logging.debug(f"批次中有效点数: {len(valid_points)}/{len(point_batch)}")

    for lat, lon, country_iso in valid_points:
        try:
            # 加载该点的能耗数据
...
def calculate_national_energy(point_coords, population_df, point_to_country):
    """计算每个国家的总能耗（使用点-国家映射）"""
    ...
    num_processes = min(num_cores * 2, 16)  # 增加最大进程数到16
    batch_size = 200  # 增加每批处理点数到200，减少进程间通信开销
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        process_func = partial(process_point_batch, point_to_country=point_to_country)
...
def main():
    ...
    try:
        ...
        point_coords = load_grid_point_results(point_to_country)

        # 2. 计算国家能耗
        logging.info("=== 第二步：计算国家能耗 ===")
        national_energy_results, national_population = calculate_national_energy(
            point_coords, population_df, point_to_country)
