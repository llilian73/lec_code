"""
全球国家级别能耗聚合计算工具 - Namibia (NA) 专用处理版本

功能概述：
本工具专门用于重新计算 Namibia (NA) 的能耗数据。由于 NA 会被 pandas 默认识别为缺失值，
导致原始计算中 Namibia 的数据丢失。本工具通过特殊的 NA 值处理，重新计算 Namibia 的能耗数据。

处理流程：
1. 从 point_country_mapping.csv 中提取 NA 对应的所有网格点坐标
2. 加载这些网格点的能耗数据
3. 聚合计算 Namibia 的总能耗和人均能耗
4. 保存到对应年份的 Africa/summary 和 Africa/summary_p 目录
...
def load_point_country_mapping():
    ...
    # 重要：keep_default_na=False 防止 'NA' (Namibia) 被识别为缺失值
    ...
    na_count = len(mapping_df[mapping_df['Country_Code'] == 'NA'])
    if na_count > 0:
        logging.info(f"✓ Namibia (NA) 数据已正确加载: {na_count} 个点")
    else:
        logging.warning("⚠ 未找到 Namibia (NA) 的数据")
...
def save_namibia_results(final_results, national_population, summary_dir, summary_p_dir, year):
    """保存 Namibia 的结果到文件"""
    logging.info("开始保存 Namibia 结果...")
    
    # Namibia 的国家名称
    country_name = 'Namibia'
    country_iso = 'NA'
    
    if country_name not in final_results:
        logging.error(f"final_results 中没有找到 {country_name} 的数据")
        return
    ...
    # 获取人口数据
    population = 0
    if 'NA' in national_population['country'].values:
        population = national_population[national_population['country'] == 'NA']['total_population'].iloc[0]
        logging.info(f"Namibia 人口: {population:,.0f}")
    else:
        logging.warning("未找到 Namibia 的人口数据")
    ...
    for i, case in enumerate(cases):
        if i == 0:  # ref case
            ...
            # 计算节能率
            total_reduction = (ref_total - total_demand[i]) / ref_total * 100 if ref_total > 0 else 0
            heating_reduction = (ref_heating - heating_demand[i]) / ref_heating * 100 if ref_heating > 0 else 0
            cooling_reduction = (ref_cooling - cooling_demand[i]) / ref_cooling * 100 if ref_cooling > 0 else 0
    ...
    for i, case in enumerate(cases):
        if i == 0:  # ref case
            ...
            # 计算节能率
            total_reduction_p = (ref_total_p - total_demand_p[i]) / ref_total_p * 100 if ref_total_p > 0 else 0
            heating_reduction_p = (ref_heating_p - heating_demand_p[i]) / ref_heating_p * 100 if ref_heating_p > 0 else 0
            cooling_reduction_p = (ref_cooling_p - cooling_demand_p[i]) / ref_cooling_p * 100 if ref_cooling_p > 0 else 0
    ...
    # 保存文件
    summary_file = os.path.join(summary_dir, f"NA_{year}_summary_results.csv")
    summary_p_file = os.path.join(summary_p_dir, f"NA_{year}_summary_p_results.csv")
    
    summary_df.to_csv(summary_file, encoding='utf-8-sig')
    summary_p_df.to_csv(summary_p_file, encoding='utf-8-sig')
    
    logging.info(f"✓ 保存成功:")
    logging.info(f"  - {summary_file}")
    logging.info(f"  - {summary_p_file}")
    logging.info(f"  总能耗: {ref_total:.2f} TWh")
    logging.info(f"  人均能耗: {ref_total_p:.2f} kWh/person")
...
def main():
    """主函数 - 专门计算 Namibia (NA) 的数据"""
    logging.info("开始计算 Namibia (NA) 的能耗数据...")
    ...
    try:
        ...
        # 只提取 Namibia (NA) 的点
        na_mapping = mapping_df[mapping_df['Country_Code'] == 'NA']
        logging.info(f"Namibia (NA) 共有 {len(na_mapping)} 个点")
        
        if len(na_mapping) == 0:
            logging.error("未找到 Namibia (NA) 的任何点，程序退出")
            return
        
        # 创建点到国家的映射（只包含NA）
        point_to_country = {}
        for _, row in na_mapping.iterrows():
            lat = row['lat']
            lon = row['lon']
            point_to_country[(lat, lon)] = 'NA'
        
        logging.info(f"创建了 {len(point_to_country)} 个 Namibia 点的映射")

        # 2. 处理每个年份的数据
        for year in YEARS:
            logging.info(f"\n{'='*60}")
            logging.info(f"处理 {year} 年 Namibia 数据")
            logging.info(f"{'='*60}")
            ...
            # 只处理 Namibia 的点
            logging.info(f"提取 {year} 年 Namibia 的网格点坐标...")
            na_point_coords = []
            for lat, lon in point_to_country.keys():
                # 检查该点的能耗文件是否存在
                base_filename = f"point_lat{lat:.3f}_lon{lon:.3f}"
                cooling_path = os.path.join(year_grid_dir, f"{base_filename}_cooling.csv")
                heating_path = os.path.join(year_grid_dir, f"{base_filename}_heating.csv")
                
                if os.path.exists(cooling_path) and os.path.exists(heating_path):
                    na_point_coords.append((lat, lon))
            
            logging.info(f"找到 {len(na_point_coords)} 个有效的 Namibia 网格点（有能耗数据）")
            
            if len(na_point_coords) == 0:
                logging.warning(f"{year}年没有找到 Namibia 的有效网格点，跳过")
                continue

            # 计算 Namibia 的国家能耗
            logging.info(f"=== 计算 {year} 年 Namibia 能耗 ===")
            national_energy_results, national_population = calculate_national_energy(
                na_point_coords, population_df, point_to_country, year_grid_dir)
            ...
            # 保存结果到 Africa 大洲目录
            logging.info(f"=== 保存 {year} 年 Namibia 结果 ===")
            africa_output_dir = os.path.join(year_output_dir, 'Africa')
            os.makedirs(africa_output_dir, exist_ok=True)
            
            summary_dir = os.path.join(africa_output_dir, 'summary')
            summary_p_dir = os.path.join(africa_output_dir, 'summary_p')
            os.makedirs(summary_dir, exist_ok=True)
            os.makedirs(summary_p_dir, exist_ok=True)
            
            # 保存 Namibia 的结果
            save_namibia_results(final_results, national_population, summary_dir, summary_p_dir, year)
