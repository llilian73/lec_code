...
def save_colorbar_only(cmap, vmin, vmax, label, save_path, cmap_key, map_type):
    """单独导出colorbar为PNG
    Args:
        cmap: 颜色映射
        vmin, vmax: 数值范围
...
def plot_average_map(avg_data, shapefile_path, save_path, case_num, code2_to_code3):
    ...
    manual_vmin_vmax = {
        'total_difference': (0, 900),
        'cooling_difference': (0, 900),
        'heating_difference': (0, 500)
    }
    reduction_vmin_vmax = {
...
if __name__ == "__main__":
    data_dir = r"Z:\local_environment_creation\energy_consumption_gird\result\result\figure_maps_and_data\not_capita\data"
    ...
    # 生成difference类型的colorbar
    difference_types = [
        ('total_difference', 'total', 'Total Demand Difference (TWh)'),
        ('cooling_difference', 'cooling', 'Cooling Demand Difference (TWh)'),
        ('heating_difference', 'heating', 'Heating Demand Difference (TWh)')
    ]
    for col, cmap_key, label in difference_types:
        vmin, vmax = manual_vmin_vmax[col]
        save_colorbar_only(colormaps[cmap_key], vmin, vmax, label, output_dir, cmap_key, 'difference')
    
    # 生成reduction类型的colorbar
    reduction_types = [
