...
def calculate_average_data(data_dir, years, case_num):
    """计算每个国家5年平均值，返回dict
    Args:
        case_num: case编号，可以是数字(1-20)或'ref'
    """
    if case_num == 'ref':
        case_key = "ref_summary.csv"
    else:
    ...
    for year in years:
        ...
        for _, row in df.iterrows():
            ...
            for col in country_stats[country]:
                ...
                if pd.isna(value):
                    continue
                # 对节能率、节能量为负值的年份进行排除（energy字段不排除负值）
...
def save_average_csv(avg_data, output_path):
    ...
    # 如果存在total_energy列，按照total_energy降序排序（NaN值排在最后）
...
def get_temp_expansion(case_num):
    """根据case编号确定拓展温度范围
    Args:
...
def get_difference_vmax(temp_expansion):
    """根据拓展温度范围获取节能量范围的最大值
    Args:
        temp_expansion: 拓展温度范围 (1-4)
    Returns:
    ...
    return vmax_map.get(temp_expansion, 1400)

def save_colorbar_only(cmap, vmin, vmax, label, save_path, cmap_key, map_type, temp_expansion=None):
    """单独导出colorbar为PNG
    Args:
        cmap: 颜色映射
        vmin, vmax: 数值范围
        label: colorbar标签
        save_path: 保存路径
        cmap_key: 颜色映射键（'total', 'cooling', 'heating'）
        map_type: 地图类型（'difference' 或 'reduction'）
        temp_expansion: 拓展温度范围 (1-4)，如果为None则不包含在文件名中
    """
    ...
    if map_type == 'difference':
        if temp_expansion is not None:
            output_file = os.path.join(save_path, f"{cmap_key}_demand_difference_colorbar_temp{temp_expansion}.png")
        else:
            output_file = os.path.join(save_path, f"{cmap_key}_demand_difference_colorbar.png")
    else:
        output_file = os.path.join(save_path, f"{cmap_key}_demand_reduction_colorbar.png")
    
    plt.savefig(output_file, format="png", bbox_inches='tight', dpi=600)
    print(f"已保存colorbar: {output_file}")
    plt.close()
...
def plot_average_map(avg_data, shapefile_path, save_path, case_num, code2_to_code3):
    ...
    # 根据case编号确定拓展温度范围
    temp_expansion = get_temp_expansion(case_num)
    vmax = get_difference_vmax(temp_expansion)
    # 根据拓展温度范围设置节能量范围（总、制冷、供暖都使用相同的最大值）
    manual_vmin_vmax = {
        'total_difference': (0, vmax),
        'cooling_difference': (0, vmax),
        'heating_difference': (0, vmax)
    }
    reduction_vmin_vmax = {
...
if __name__ == "__main__":
    ...
    # 生成所有类型的colorbar（为每个拓展温度范围生成difference类型的colorbar）
    print("正在生成colorbar...")
    colormaps = create_custom_colormaps()
    reduction_vmin_vmax = {
        'total_reduction': (0, 100),
    ...
    # 为每个拓展温度范围生成difference类型的colorbar
    difference_types = [
        ('total_difference', 'total', 'Total Demand Difference (kWh/person)'),
        ('cooling_difference', 'cooling', 'Cooling Demand Difference (kWh/person)'),
        ('heating_difference', 'heating', 'Heating Demand Difference (kWh/person)')
    ]
    # 拓展温度范围：1-4
    for temp_expansion in range(1, 5):
        vmax = get_difference_vmax(temp_expansion)
        for col, cmap_key, label in difference_types:
            save_colorbar_only(colormaps[cmap_key], 0, vmax, label, output_dir, cmap_key, 'difference', temp_expansion)
    
    # 生成reduction类型的colorbar（reduction类型不需要按拓展温度范围区分）
    reduction_types = [
        ('total_reduction', 'total', 'Total Demand Reduction (%)'),
        ('cooling_reduction', 'cooling', 'Cooling Demand Reduction (%)'),
        ('heating_reduction', 'heating', 'Heating Demand Reduction (%)')
    ]
    for col, cmap_key, label in reduction_types:
        vmin, vmax = reduction_vmin_vmax[col]
        save_colorbar_only(colormaps[cmap_key], vmin, vmax, label, output_dir, cmap_key, 'reduction')
    
    print("Colorbar生成完成！\n")
    
    # 先处理ref工况
    print("处理 ref 工况...")
    ref_args = ('ref', data_dir, years, shapefile_path, output_dir, CODE2_TO_CODE3)
    ref_result = process_single_case(ref_args)
    print(f"  {ref_result}\n")
