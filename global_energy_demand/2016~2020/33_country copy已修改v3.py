...
def group_points_by_quadrant(point_to_country, block_to_path):
    """将点和空间块按象限分组，返回 {quadrant: {block_name: [(lat, lon), ...]}}"""
    logging.info("按象限分组点和空间块...")
    
    # 创建象限到空间块的映射
    quadrant_to_blocks = {0: {}, 1: {}, 2: {}, 3: {}}
    
    # 统计每个象限的点数和国家
    quadrant_stats = {0: {'points': 0, 'countries': set()}, 
                      1: {'points': 0, 'countries': set()}, 
                      2: {'points': 0, 'countries': set()}, 
                      3: {'points': 0, 'countries': set()}}
    ...
    for (lat, lon), country_code in point_to_country.items():
        ...
        if block_name in block_to_path:
            if block_name not in quadrant_to_blocks[quadrant]:
                quadrant_to_blocks[quadrant][block_name] = []
            quadrant_to_blocks[quadrant][block_name].append((lat, lon))
            
            # 更新统计信息
            quadrant_stats[quadrant]['points'] += 1
            if country_code is not None:
                quadrant_stats[quadrant]['countries'].add(country_code)
    
    # 输出统计信息
    for quadrant in range(4):
        blocks_count = len(quadrant_to_blocks[quadrant])
        points_count = quadrant_stats[quadrant]['points']
        countries_count = len(quadrant_stats[quadrant]['countries'])
        logging.info(f"{get_quadrant_name(quadrant)}: {blocks_count} 个空间块, {points_count} 个点, {countries_count} 个国家")
    
    return quadrant_to_blocks
...
def combine_blocks_into_batches(block_to_points, batch_point_limit=300):
...
def calculate_national_energy_for_quadrant(quadrant_blocks, point_to_country, block_to_path):
    """计算单个象限内每个国家的总能耗和逐时能耗"""
    if len(quadrant_blocks) == 0:
        logging.warning(f"象限内没有有效的空间块，返回空结果")
        return {}, {}
    
    # 将空间块组合成批次（每批最多500个点，减少内存占用）
    block_batches = combine_blocks_into_batches(quadrant_blocks, batch_point_limit=300)
    ...
    # 减少进程数以降低内存压力（内存不足时减少并发）
    num_processes = min(num_cores, 8)  # 减少到8个进程以降低内存压力

    logging.info(f"CPU核心数: {num_cores}")
    logging.info(f"使用进程数: {num_processes} (已减少以降低内存压力)")
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        ...
        with tqdm(total=len(batch_args), desc="处理空间块批次") as pbar:
            for batch_results, batch_hourly_results in pool.imap_unordered(process_spatial_block_batch, batch_args, chunksize=chunksize):
                ...
                # 每处理一个批次后都强制垃圾回收（内存压力大时更频繁释放）
                gc.collect()

                pbar.update(1)

    total_points = sum(len(points) for points in quadrant_blocks.values())
    logging.info(f"象限空间块批次处理完成，共处理 {total_points} 个点")

    return national_energy_results, national_hourly_results


def determine_completed_countries_in_quadrant(quadrant_blocks, point_to_country, processed_quadrants):
    """确定象限内哪些国家已完成所有点的处理
    参数:
        quadrant_blocks: 当前象限的空间块字典
        point_to_country: 点到国家的映射
        processed_quadrants: 已处理的象限集合
    返回：已完成国家的集合
    """
    # 获取当前象限内所有点的国家代码
    quadrant_countries = set()
    for block_name, points in quadrant_blocks.items():
        for lat, lon in points:
            country_code = point_to_country.get((lat, lon), None)
            if country_code is not None:
                quadrant_countries.add(country_code)
    
    # 对于每个国家，检查是否所有点都在已处理的象限内
    # 如果某个国家的所有点都在已处理的象限内，则该国家已完成
    completed_countries = set()
    
    for country_code in quadrant_countries:
        # 检查该国家的所有点是否都在已处理的象限内
        all_points_processed = True
        for (lat, lon), code in point_to_country.items():
            if code == country_code:
                point_quadrant = get_quadrant(lat, lon)
                if point_quadrant not in processed_quadrants:
...
def format_value(value, precision):
    """格式化数值：如果值为0，返回"0"，否则保留指定小数位数"""
    if abs(value) < 1e-10:  # 使用很小的阈值判断是否为0
        return "0"
    else:
    ...
    for continent, countries in continents.items():
        ...
        for country_name, country_iso2_code in countries:
            if country_name in final_results:
                ...
                # 总能耗汇总（保留13位小数，但0值不保留小数）
                summary_df = pd.DataFrame({
                    'total_demand_sum(TWh)': [format_value(x, 13) for x in total_demand],
                    'total_demand_diff(TWh)': [format_value(x, 13) for x in total_demand_diff],
                    'total_demand_reduction(%)': [format_value(x, 13) for x in total_demand_reduction],
                    'heating_demand_sum(TWh)': [format_value(x, 13) for x in heating_demand],
                    'heating_demand_diff(TWh)': [format_value(x, 13) for x in heating_demand_diff],
                    'heating_demand_reduction(%)': [format_value(x, 13) for x in heating_demand_reduction],
                    'cooling_demand_sum(TWh)': [format_value(x, 13) for x in cooling_demand],
                    'cooling_demand_diff(TWh)': [format_value(x, 13) for x in cooling_demand_diff],
                    'cooling_demand_reduction(%)': [format_value(x, 13) for x in cooling_demand_reduction]
                }, index=cases)
                ...
                summary_p_df = pd.DataFrame({
                    'total_demand_sum_p(kWh/person)': [format_value(x, 4) for x in total_demand_p],
                    'total_demand_diff_p(kWh/person)': [format_value(x, 4) for x in total_demand_diff_p],
                    'total_demand_p_reduction(%)': [format_value(x, 4) for x in total_demand_p_reduction],
                    'heating_demand_sum_p(kWh/person)': [format_value(x, 4) for x in heating_demand_p],
                    'heating_demand_diff_p(kWh/person)': [format_value(x, 4) for x in heating_demand_diff_p],
                    'heating_demand_p_reduction(%)': [format_value(x, 4) for x in heating_demand_p_reduction],
                    'cooling_demand_sum_p(kWh/person)': [format_value(x, 4) for x in cooling_demand_p],
                    'cooling_demand_diff_p(kWh/person)': [format_value(x, 4) for x in cooling_demand_diff_p],
                    'cooling_demand_p_reduction(%)': [format_value(x, 4) for x in cooling_demand_p_reduction]
                }, index=cases)
...
def main():
    ...
    try:
        ...
        for year in YEARS:
            ...
            # 按象限处理国家能耗（每个象限处理完后立即保存已完成的国家数据并释放内存）
            logging.info(f"=== 按象限计算{year}年国家能耗 ===")
            
            # 按象限分组点和空间块
            quadrant_to_blocks = group_points_by_quadrant(point_to_country, block_to_path)
            
            # 用于累积跨象限国家的数据（这些国家的点分布在多个象限中）
            accumulated_energy_results = {}
            accumulated_hourly_results = {}
            
            # 用于记录已处理的象限
            processed_quadrants = set()
            
            # 用于累积所有已完成国家的结果（用于最终检查）
            all_final_results = {}
            
            # 按象限循环处理
            for quadrant in range(4):
                quadrant_name = get_quadrant_name(quadrant)
                logging.info(f"=== 处理{quadrant_name} ===")
                
                quadrant_blocks = quadrant_to_blocks[quadrant]
                if len(quadrant_blocks) == 0:
                    logging.info(f"{quadrant_name}没有数据，跳过")
                    processed_quadrants.add(quadrant)
                    continue
                
                # 计算该象限内的国家能耗
                quadrant_energy_results, quadrant_hourly_results = calculate_national_energy_for_quadrant(
                    quadrant_blocks, point_to_country, block_to_path)
                
                # 累积跨象限国家的数据
                cases = ['ref'] + [f'case{i}' for i in range(1, 21)]
                for country, cases_data in quadrant_energy_results.items():
                    if country not in accumulated_energy_results:
                        accumulated_energy_results[country] = {}
                        accumulated_hourly_results[country] = {}
                        for case in cases:
                            accumulated_energy_results[country][case] = {
                                'cooling_demand': 0.0,
                                'heating_demand': 0.0,
                                'total_demand': 0.0
                            }
                            if country in quadrant_hourly_results and case in quadrant_hourly_results[country]:
                                time_index = quadrant_hourly_results[country][case]['cooling'].index
                                accumulated_hourly_results[country][case] = {
                                    'cooling': pd.Series(0.0, index=time_index),
                                    'heating': pd.Series(0.0, index=time_index),
                                    'total': pd.Series(0.0, index=time_index)
                                }
                    ...
                    # 累加数据
                    for case, data in cases_data.items():
                        accumulated_energy_results[country][case]['cooling_demand'] += data['cooling_demand']
                        accumulated_energy_results[country][case]['heating_demand'] += data['heating_demand']
                        accumulated_energy_results[country][case]['total_demand'] += data['total_demand']
                    
                    for case in cases:
                        if case in quadrant_hourly_results[country]:
                            accumulated_hourly_results[country][case]['cooling'] += quadrant_hourly_results[country][case]['cooling']
                            accumulated_hourly_results[country][case]['heating'] += quadrant_hourly_results[country][case]['heating']
                            accumulated_hourly_results[country][case]['total'] += quadrant_hourly_results[country][case]['total']
                
                # 标记当前象限为已处理
                processed_quadrants.add(quadrant)
                
                # 确定哪些国家已完成所有点的处理
                completed_countries = determine_completed_countries_in_quadrant(
                    quadrant_blocks, point_to_country, processed_quadrants)
                logging.info(f"{quadrant_name}处理完成，发现 {len(completed_countries)} 个已完成的国家")
                
                # 对于已完成的国家，立即应用功率系数并保存
                if completed_countries:
                    # 提取已完成国家的累积数据
                    completed_energy_results = {country: accumulated_energy_results[country] 
                                                for country in completed_countries if country in accumulated_energy_results}
                    completed_hourly_results = {country: accumulated_hourly_results[country] 
                                               for country in completed_countries if country in accumulated_hourly_results}
                    
                    # 应用功率系数
                    logging.info(f"=== 应用{quadrant_name}已完成国家的功率系数 ===")
                    completed_final_results = apply_power_coefficients(completed_energy_results, params_dict, country_info_map)
                    completed_final_hourly_results = apply_power_coefficients_to_hourly(completed_hourly_results, params_dict, country_info_map)
                    
                    # 保存已完成国家的结果
                    logging.info(f"=== 保存{quadrant_name}已完成国家的结果 ===")
                    save_results(completed_final_results, national_population, year_output_dir, 
                               completed_final_hourly_results, year, country_info_map, mapping_df)
                    
                    # 累积到总结果中（用于最终检查）
                    all_final_results.update(completed_final_results)
                    
                    # 从累积结果中移除已完成的国家，释放内存
                    for country in completed_countries:
                        if country in accumulated_energy_results:
                            del accumulated_energy_results[country]
                        if country in accumulated_hourly_results:
                            del accumulated_hourly_results[country]
                    
                    # 强制垃圾回收
                    del completed_energy_results, completed_hourly_results
                    del completed_final_results, completed_final_hourly_results
                    gc.collect()
                    logging.info(f"{quadrant_name}已完成国家的数据已保存并释放内存，释放了 {len(completed_countries)} 个国家的数据")
                
                # 释放象限结果的内存
                del quadrant_energy_results, quadrant_hourly_results
                gc.collect()
                
                logging.info(f"{quadrant_name}处理完成")
            
            # 检查缺失的国家（传递country_info_map，避免重复读取）
            logging.info(f"=== 检查{year}年缺失的国家 ===")
            check_missing_countries(all_final_results, processed_countries, country_info_map)
            
            # 释放该年份的所有数据，为下一年的处理释放内存
            logging.info(f"=== 释放{year}年的内存数据 ===")
            del accumulated_energy_results, accumulated_hourly_results
            del all_final_results
            del quadrant_to_blocks
            del block_to_path
            # 强制垃圾回收
            gc.collect()
            logging.info(f"{year}年数据处理完成，内存已释放")
