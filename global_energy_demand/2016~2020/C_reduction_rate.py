...
def load_country_continent_mapping(mapping_file):
    ...
    """
    df = pd.read_csv(mapping_file, keep_default_na=False)
    # 新格式中列名为 Country_Code_2 和 Continent
    df['Country_Code_2'] = df['Country_Code_2'].astype(str).str.strip().str.upper()
    return dict(zip(df['Country_Code_2'], df['Continent']))
...
def add_continent_column(csv_path, output_dir, mapping_file, case_name):
    ...
    # 添加大洲列
    df['continent'] = df['country'].astype(str).str.strip().str.upper().apply(
        lambda x: get_continent_from_country_code(x, country_continent_map)
    )
...
def calculate_reduction_rate_distribution(df, output_dir, case_name):
    """计算各大洲节能率分布"""
    df = df.copy()
    df['total_reduction'] = pd.to_numeric(df['total_reduction'], errors='coerce')
    # 定义节能率区间（10个区间，10%一档）
    ranges = [
        ('0~10%', 0, 10),
        ('10~20%', 10, 20),
    ...
    for continent in df['continent'].unique():
        ...
        continent_data.loc[nan_mask, 'total_reduction'] = 0  # 将NaN设为0，归入0~10%区间
    ...
    if not result_df.empty:
        sum_by_continent = result_df.groupby('continent')['rate'].sum()
        for continent, total_rate in sum_by_continent.items():
            if abs(total_rate - 100) > 0.1:  # 允许小的浮点误差
                print(f"警告: {case_name} 中 {continent} 各区间占比之和为 {total_rate:.2f}%，未达到100%。")
...
def main():
    # 输入目录
    input_dir = r"Z:\local_environment_creation\energy_consumption_gird\result\result\figure_maps_and_data\per_capita\data\average"
    
    # 国家大洲映射文件
    mapping_file = r"Z:\local_environment_creation\energy_consumption_gird\result\country_population_2020.csv"
