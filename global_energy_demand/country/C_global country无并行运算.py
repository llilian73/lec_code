...
def process_region_by_year(bait_file, params_dict, country_pop_dict, continent, base_output_dir, error_recorder, processed_countries, year):
    """按年份处理单个BAIT文件"""
    try:
        # 从BAIT文件中获取需要处理的国家代码
        ...
        for region_code in country_codes:
            try:
                ...
                if not country_name:
                    error_msg = f"无法找到国家代码 {region_code} 对应的国家全称"
                    logging.error(error_msg)
                    error_recorder.add_error(continent, None, region_code, error_msg)
                    continue
                 # 打印国家代码和名称（已注释）
                # print(f"国家代码: {region_code}, 国家名称: {country_name}")

                # logging.info(f"正在计算 {country_name}（{region_code}）")
                ...
                if region_code in params_dict:
                    ...
                    # logging.info(f"{country_name}（{region_code}）使用默认参数")

                # 获取国家人口数据
                population = country_pop_dict.get(country_name)
                if not population:
                    error_msg = f"未找到 {country_name} 的人口数据"
                    logging.warning(error_msg)
                    error_recorder.add_error(continent, country_name, region_code, error_msg)
                    continue
                ...
                # 在成功计算完一个国家后添加到已处理列表
                processed_countries.add_country(continent, region_code, country_name)

            except Exception as e:
                error_msg = f"处理国家 {region_code} 时出错: {str(e)}"
                logging.error(error_msg)
                error_recorder.add_error(continent, country_name, region_code, error_msg)

    except Exception as e:
        error_msg = f"处理BAIT文件时出错: {str(e)}"
        logging.error(error_msg)
...
def main():
    ...
    for year in year_progress:
        ...
        for continent in continents:
            logging.info(f"正在计算 {continent} - {year} 年")
            ...
            for file in os.listdir(continent_dir):
                if file.startswith("demand_ninja_") and file.endswith("_BAIT.csv"):
                    ...
                    process_region_by_year(bait_file, params_dict, country_pop_dict, continent, 
                                         output_base_dir, error_recorder, processed_countries, year)
            
            logging.info(f"{continent} - {year} 年计算完成")
        
        logging.info(f"{year} 年所有大洲计算完成")
