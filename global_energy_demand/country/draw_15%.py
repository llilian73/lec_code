...
def load_15_percent_data(csv_path):
    """读取15%节能率数据"""
    df = pd.read_csv(csv_path)
    return df


def plot_15_percent_line_chart(df, save_path):
    """绘制15%节能率点线图"""
    # 定义温度和case的对应关系
    temp_cases = [
    ...
    for i, temp_data in enumerate(temp_cases):
        ...
        for case in cases:
            ...
            if not case_data.empty:
                temp_values.append(case_data['>15%'].iloc[0])
            else:
...
def main():
    # 输入文件路径
    input_file = r"Z:\local_environment_creation\energy_consumption\2016-2020result\reduction_rate\15%\15_percent_reduction_summary.csv"
    ...
    # 读取数据
    print("正在读取数据...")
    df = load_15_percent_data(input_file)
    print(f"读取了 {len(df)} 条数据")
    
    # 生成输出文件名
    output_filename = "15_percent_line_chart.pdf"
    output_path = os.path.join(output_dir, output_filename)
    
    # 绘制图表
    print("正在绘制图表...")
    plot_15_percent_line_chart(df, output_path)
