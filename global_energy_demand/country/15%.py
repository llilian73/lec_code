...
def process_15_percent_reduction(input_dir, output_dir):
    """统计总节能率大于等于15%的国家占比"""
    # 创建输出目录
    ...
    for csv_file in csv_files:
        ...
        # 统计节能率大于等于15%的国家数
        countries_above_15 = len(df_clean[df_clean['total_reduction'] >= 15])
        
        # 计算百分比
        percentage_above_15 = (countries_above_15 / total_countries) * 100 if total_countries > 0 else 0
        
        results.append({
            'case': case_name,
            '>15%': percentage_above_15
        })
        
        print(f"  {case_name}: {countries_above_15}/{total_countries} = {percentage_above_15:.2f}%")
    ...
    # 保存结果
    output_filename = "15_percent_reduction_summary.csv"
...
def main():
    ...
    # 输出目录（保存15%统计结果）
    output_dir = r"Z:\local_environment_creation\energy_consumption\2016-2020result\reduction_rate\15%"
    
    print("开始统计节能率大于等于15%的国家占比...")
    process_15_percent_reduction(input_dir, output_dir)
    print("\n统计完成！")
