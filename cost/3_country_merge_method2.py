...
# 配置参数
INPUT_FILE = r"Z:\local_environment_creation\cost\method2\country_energy_power_with_cost.csv"
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "country_cost.csv")
...
def load_cost_data():
    ...
    cost_df = read_csv_with_encoding(INPUT_FILE, keep_default_na=False)
...
def load_point_country_mapping():
    ...
    mapping_df = read_csv_with_encoding(POINT_COUNTRY_MAPPING_FILE, keep_default_na=False)
...
def load_country_population():
    ...
    population_df = read_csv_with_encoding(COUNTRY_POPULATION_FILE, keep_default_na=False)
