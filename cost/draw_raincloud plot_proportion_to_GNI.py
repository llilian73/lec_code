...
功能概述：
本工具用于绘制成本占GNI比例的云雨图，按收入组分组显示数据分布。

输入数据：
- 文件：Z:\local_environment_creation\cost\country_cost_proportion_to_GNI.csv
- 数据列：IncomeGroup, Country Name, Country Code_2, total_cost, GNI_per, propotion_to_GNI

输出结果：
- PDF图片：Z:\local_environment_creation\cost\method2\raincloud_plot_proportion_to_GNI.pdf
- CSV统计文件：Z:\local_environment_creation\cost\method2\b_proportion_to_GNI_statistics.csv
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "raincloud_plot_proportion_to_GNI.pdf")
...
def load_data():
    ...
    if 'propotion_to_GNI' not in df.columns:
        raise ValueError("CSV文件中缺少'propotion_to_GNI'列")
    
    # 去除propotion_to_GNI或IncomeGroup为空值的行
    original_count = len(df)
    df = df.dropna(subset=['propotion_to_GNI', 'IncomeGroup'])
...
def save_statistics_to_csv(df):
    ...
    for income_group in income_groups:
        data = df[df['IncomeGroup'] == income_group]['propotion_to_GNI'].values
    ...
    # 保存到CSV文件
    output_csv_path = os.path.join(OUTPUT_DIR, "b_proportion_to_GNI_statistics.csv")
...
def plot_raincloud(df):
    ...
    ax.set_ylabel('Proportion to GNI', fontsize=16, labelpad=15)
    ax.set_xlabel('Income Group', fontsize=16, labelpad=15)
    # ax.set_title('Raincloud Plot of Cost Proportion to GNI by Income Group', 
    #              fontsize=14, pad=20)
...
def print_statistics(df):
    ...
    stats_df = df.groupby('IncomeGroup')['propotion_to_GNI'].agg([
        ('count', 'count'),
        ('mean', 'mean'),
        ('median', 'median'),
        ('min', 'min'),
