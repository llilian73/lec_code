...
功能概述：
本工具用于根据PP数据按大洲绘制分组百分比堆叠柱状图。

输入数据：
- 文件：Z:\local_environment_creation\cost\country_yearly_per_cost_USD_PP.csv
- 数据列：Continent, Country_Code, Country_Name, equipment_cost, maintenance_cost, total_cost_temp, save_elec_cost, PP

输出结果：
- PDF图片：Z:\local_environment_creation\cost\PP_distribution_chart.pdf
"""
...
def load_pp_data(csv_path):
    ...
    print(f"过滤Unknown大洲和PP空值后，剩余 {filtered_count} 行（过滤了 {original_count - filtered_count} 行）")
...
def main():
    ...
    # 输出文件
    output_file = r"Z:\local_environment_creation\cost\method2\PP_distribution_chart.pdf"
