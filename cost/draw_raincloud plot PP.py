...
功能概述：
本工具用于绘制投资回收期（PP）的云雨图，按收入组分组显示数据分布。
...
输出结果：
- PDF图片：Z:\local_environment_creation\cost\raincloud_plot_PP.pdf
"""
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "raincloud_plot_PP.pdf")
...
def load_data():
    ...
    # 去除PP或IncomeGroup为空值的行
    original_count = len(df)
    df = df.dropna(subset=['PP', 'IncomeGroup'])
...
def plot_raincloud(df):
    ...
    for i, income_group in enumerate(income_groups):
        data = df[df['IncomeGroup'] == income_group]['PP'].values
    ...
    ax.set_ylabel('Payback Period (year)', fontsize=12, fontweight='bold')
    ax.set_xlabel('Income Group', fontsize=12, fontweight='bold')
    ax.set_title('Raincloud Plot of Payback Period by Income Group', 
...
def print_statistics(df):
    ...
    stats_df = df.groupby('IncomeGroup')['PP'].agg([
        ('count', 'count'),
        ('mean', 'mean'),
        ('median', 'median'),
        ('min', 'min'),
