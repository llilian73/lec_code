"""
全球人均碳排放量计算工具（2016-2020年）

功能概述：
本工具用于计算全球各个国家的人均碳排放量，基于能耗数据和碳排放因子。将人均能耗数据转换为碳排放数据，支持多种节能工况的碳排放效果分析。
...
1. 能耗数据：
   ...
   - 数据位置：{年份}\{大洲}\summary_p\{国家代码}_{年份}_summary_p_results.csv
   - 关键列：total_demand_sum_p(kWh/person) - 人均总能耗
...
1. 国家级别碳排放数据：
   ...
   - 文件格式：{国家代码}_{年份}_carbon_p_emission.csv
   - 包含列：
     * carbon_emission(kgCO2/person) - 人均碳排放量
     * carbon_emission_reduction(kgCO2/person) - 相对于ref的碳排放减少量
     * carbon_emission_reduction(%) - 碳排放减少比例

2. 汇总数据：
   - 输出目录：Z:\local_environment_creation\carbon_emission\result\{年份}\
   - 文件格式：{case_name}_summary.csv
   - 包含列：Country_Code_2, carbon_emission(kgCO2/person), carbon_emission_reduction(kgCO2/person), carbon_emission_reduction(%)

计算公式：
- 人均碳排放量 = 人均能耗(kWh/person) × 碳排放因子(gCO2/kWh) × 10^-3
...
def process_country_data(country_code, energy_file, emission_factor, output_file):
    ...
    try:
        ...
        # 计算碳排放量（kWh/person * gCO2/kWh * 10^-3 = kgCO2/person）
        results['carbon_emission(kgCO2/person)'] = df['total_demand_sum_p(kWh/person)'] * emission_factor * 1e-3
        
        # 计算减少的碳排放量
        ref_emission = results.loc['ref', 'carbon_emission(kgCO2/person)']
        results['carbon_emission_reduction(kgCO2/person)'] = ref_emission - results['carbon_emission(kgCO2/person)']
        
        # 计算减少比例
        results['carbon_emission_reduction(%)'] = (results['carbon_emission_reduction(kgCO2/person)'] / ref_emission) * 100
...
def process_year(year, energy_base_dir, output_base_dir, emission_factors_by_year, error_log):
    ...
    for continent in continents:
        ...
        # 能耗数据目录
        energy_dir = os.path.join(energy_base_dir, str(year), continent, 'summary_p')
        ...
        energy_files = [f for f in os.listdir(energy_dir) 
                       if f.endswith(f'_{year}_summary_p_results.csv')]
        ...
        for file in energy_files:
            ...
            output_file = os.path.join(continent_output_dir, f"{country_code}_{year}_carbon_p_emission.csv")
                ...
                for case in cases:
                    if case in results_df.index:
                        all_countries_results[case].append({
                            ...
                            'carbon_emission(kgCO2/person)': results_df.loc[case, 'carbon_emission(kgCO2/person)'],
                            'carbon_emission_reduction(kgCO2/person)': results_df.loc[case, 'carbon_emission_reduction(kgCO2/person)'],
    ...
    print(f"\n保存 {year} 年的汇总数据...")
    # 创建capita子目录
    capita_output_dir = os.path.join(year_output_dir, 'capita')
    os.makedirs(capita_output_dir, exist_ok=True)
    ...
    for case in cases:
        if all_countries_results[case]:
            ...
            summary_file = os.path.join(capita_output_dir, f"{case}_summary.csv")
            summary_df.to_csv(summary_file, index=False)
            print(f"  已保存: capita/{case}_summary.csv ({len(summary_df)}个国家)")
