...
功能概述：
本工具用于将计算得到的人均碳排放数据转换为全球热图，展示不同工况下各国的碳排放量和减排效果。支持碳排放量热图和减排量热图两种可视化方式。

输入数据：
1. 人均碳排放数据：
   - 基础目录：D:\workstation\carbon emission\global_capita
   - 按大洲分类：Africa, Asia, Europe, North America, Oceania, South America
   - 数据位置：{大洲}\{国家代码}_carbon_p_emission.csv
   - 关键列：
...
输出结果：
1. 碳排放量热图（emission目录）：
   - 输出目录：D:\workstation\carbon emission\global_capita\emission\
   - 文件格式：global_{工况}_p_emission.pdf
   - 包含工况：ref, case1-case9（共10个文件）
   - 显示内容：各国人均碳排放量（kgCO2/person）

2. 减排量热图（reduction目录）：
   - 输出目录：D:\workstation\carbon emission\global_capita\reduction\
   - 文件格式：global_{工况}_p_reduction.pdf
   - 包含工况：case1-case9（共9个文件）
   - 显示内容：各国相对于ref的碳排放减少量（kgCO2/person）
...
与draw_global.py的区别：
- 本工具处理人均数据（kgCO2/person）
- draw_global.py处理总量数据（tCO2）
- 输入目录：global_capita vs global
- 输出目录：global_capita\emission vs global\emission
- 单位差异：kgCO2/person vs tCO2
            ...
            if file.endswith('_carbon_p_emission.csv'):
                ...
                for case in ['ref', 'case1', 'case2', 'case3', 'case4', 
                    ...
                    if not case_row.empty:
                        case_data[f'{case}_emission'] = case_row['carbon_emission(kgCO2/person)'].iloc[0]
                        if case != 'ref':  # 对于非ref工况，获取减少量
                            case_data[f'{case}_reduction'] = case_row['carbon_emission_reduction(kgCO2/person)'].iloc[0]
...
def main():
    ...
    # 设置路径
    base_dir = r"D:\workstation\carbon emission\global_capita"
    ...
    print(f"所有工况最大碳排放值: {vmax_emission} kgCO2/person")
    ...
    print(f"最大碳排放减少值: {vmax_reduction} kgCO2/person")
    ...
    for case in cases:
        ...
        create_choropleth(
            ...
            f'Global {case} Case Carbon Emission (kgCO2/person)',
            os.path.join(emission_dir, f'global_{case}_p_emission.pdf'),
            vmax=vmax_emission,
            is_reduction=False
        )
    ...
    for case in reduction_cases:
        ...
        create_choropleth(
            ...
            f'Global {case} Case Carbon Emission Reduction (kgCO2/person)',
            os.path.join(reduction_dir, f'global_{case}_p_reduction.pdf'),
            vmax=vmax_reduction,
            is_reduction=True
        )
