"""
全球总碳排放量平均地图绘制工具（2016-2020年）

功能概述：
本工具用于计算全球各个国家5年（2016-2020）的平均碳排放量，并绘制热图展示碳排放总量、减少量和减少率。
...
输出结果：
1. 平均数据CSV文件：
   - {case_name}_summary_average.csv：各国5年平均碳排放数据
   - 格式：Country_Code_2, carbon_emission(tCO2), carbon_emission_reduction(tCO2), carbon_emission_reduction(%)

2. 热图PDF文件：
   - carbon_emission_map_{case_name}_average.pdf：碳排放总量热图
   - carbon_emission_reduction_map_{case_name}_average.pdf：碳排放减少量热图
   - carbon_emission_reduction_rate_map_{case_name}_average.pdf：碳排放减少率热图
"""
...
def calculate_average_data(data_dir, years, case_name):
    ...
    for year in years:
        year_path = os.path.join(data_dir, str(year), 'notcapita', f"{case_name}_summary.csv")
        ...
        for _, row in df.iterrows():
            ...
            if pd.notna(emission_value) and emission_value >= 0:
                country_stats[country]['carbon_emission(tCO2)'].append(emission_value)
            
            # 处理减少量（只对非ref工况）
            if case_name != 'ref':
                reduction_value = pd.to_numeric(row.get('carbon_emission_reduction(tCO2)', np.nan), errors='coerce')
                if pd.notna(reduction_value) and reduction_value >= 0:
                    country_stats[country]['carbon_emission_reduction(tCO2)'].append(reduction_value)
...
def plot_carbon_maps(avg_data, shapefile_path, save_path, case_name, code2_to_code3):
    ...
    if case_name != 'ref':
        ...
        for code3, row in country_data_code3.items():
            val = row.get('carbon_emission(tCO2)', np.nan)
            if pd.notna(val):
                # 直接使用GID_0（三字母代码）匹配
        ...
        if 'CN' in country_data:
            ...
            cn_val = cn_row.get('carbon_emission(tCO2)', np.nan)
            if pd.notna(cn_val):
                # 获取中国的三字母代码
        ...
        # 计算vmax（使用数据最大值）
        vmax = df_avg['carbon_emission(tCO2)'].max() if not df_avg['carbon_emission(tCO2)'].isna().all() else 1000
        
        world.plot(column='value',
                   ax=ax,
                   missing_kwds={'color': 'lightgrey'},
                   legend=True,
                   legend_kwds={'label': 'Carbon Emission (tCO2)',
        ...
        plt.title(f'Carbon Emission (tCO2) - {case_name.capitalize()} - 5-Year Average', fontsize=16)
        ax.axis('off')
    ...
    if case_name != 'ref':
        ...
        # 使用三字母代码匹配
        for code3, row in country_data_code3.items():
            val = row.get('carbon_emission_reduction(tCO2)', np.nan)
            if pd.notna(val):
                # 直接使用GID_0（三字母代码）匹配
                world.loc[world['GID_0'] == code3, 'value'] = val
        ...
        if 'CN' in country_data:
            cn_row = country_data['CN']
            cn_val = cn_row.get('carbon_emission_reduction(tCO2)', np.nan)
            if pd.notna(cn_val):
                # 获取中国的三字母代码
                chn_code3 = code2_to_code3.get('CN', 'CHN')
                if chn_code3:
        ...
        # 计算vmax（使用数据最大值）
        vmax = df_avg['carbon_emission_reduction(tCO2)'].max() if not df_avg['carbon_emission_reduction(tCO2)'].isna().all() else 1000
        
        world.plot(column='value',
                   ax=ax,
                   missing_kwds={'color': 'lightgrey'},
                   legend=True,
                   legend_kwds={'label': 'Carbon Emission Reduction (tCO2)',
                               'orientation': 'horizontal',
                               'shrink': 0.6, 'pad': 0.02},
                   ...
                   vmax=vmax)
        add_tropic_lines(ax, world)
        plt.title(f'Carbon Emission Reduction (tCO2) - {case_name.capitalize()} - 5-Year Average', fontsize=16)
        ax.axis('off')
...
if __name__ == "__main__":
    ...
    # 输出目录
    data_output_dir = r"Z:\local_environment_creation\carbon_emission\2016-2020\figure_maps_and_data\not_capita\data"
    figure_output_dir = r"Z:\local_environment_creation\carbon_emission\2016-2020\figure_maps_and_data\not_capita\figure"
